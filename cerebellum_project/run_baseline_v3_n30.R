...
cat("==========================================\n")
cat(" RUNNING BASELINE v3 (N=30) AND FINAL METRICS\n")
cat("==========================================\n\n")
...
cat("Compiling Baseline v3...\n")
mod_v3 <- cmdstan_model("/home/DCCS5/cerebellum_project/src/stan/baseline_v3.stan", cpp_options = list(stan_threads = TRUE))

cat("Sampling Baseline v3 (N=30) ...\n")
fit_v3 <- mod_v3$sample(
  data = stan_data_v3,
  chains = 4,
  parallel_chains = 2,
  threads_per_chain = 4,
...
fit_v3$save_output_files(dir = "/home/DCCS5/cerebellum_project/results/", basename = "fit_v3_n30")

summ_v3 <- fit_v3$summary()
cat("Baseline v3 Diagnostics -> Max Rhat:", max(summ_v3$rhat, na.rm=TRUE), "| Min Bulk ESS:", min(summ_v3$ess_bulk, na.rm=TRUE), "\n")
loo_v3 <- fit_v3$loo()
saveRDS(loo_v3, "/home/DCCS5/cerebellum_project/results/loo_v3_n30.rds")

loo_m012 <- readRDS("/home/DCCS5/cerebellum_project/results/loo_m012_n30.rds")
cat("\n==========================================\n")
cat(" LOO-CV COMPARISON (N=30)\n")
cat("==========================================\n")
comp <- loo_compare(list(M012 = loo_m012, Baselinev3 = loo_v3))
print(comp)
cat("\n")
...
tnd_cap <- pmin(stan_data_v3$min_rt - 0.05, 3.69)
