...
Rcpp::sourceCpp("magi_swarm_003.cpp")
Rcpp::sourceCpp("magi_swarm_001.cpp") # To get baseline WFPT
...
cat("Phase 2: Evaluating Swarm 003...\n")
run_cand <- function(s_idx, h) {
    tryCatch({
        d <- d_list[[s_idx]]
        obj <- function(p){v<-get_nll_swarm_003(p,h,d$Boundary+1,d$Reward,d$RT);if(is.nan(v))1e6 else v}
        res <- tryCatch(cma_es(rep(0,7),obj,control=list(maxit=50,sigma=0.5)),error=function(e)list(par=rep(0,7),value=NA))
        data.frame(SubjectID=s_idx, NLL=res$value, Model="Swarm003")
    }, error=function(e) data.frame(SubjectID=s_idx, NLL=NA, Model="Swarm003"))
}

# h_swarm <- c(lambda_sparse, beta_ising, K_sa, theta_io, lambda_MF)
h_swarm <- c(1e-3, 5e-4, 25, 0.15, 50.0)
df_cand <- bind_rows(mclapply(1:S, run_cand, h=h_swarm, mc.cores=CORES))

df_all <- bind_rows(df_base, df_cand) %>% drop_na()
df_all$Model <- factor(df_all$Model, levels=c("Base", "Swarm003"))

cat("\n========== STATISTICAL SUPREMACY TEST ==========\n")
if(nrow(df_all) > 10) {
    mod <- lmer(NLL ~ Model + (1 | SubjectID), data=df_all)
    sm <- summary(mod)
    est <- sm$coefficients["ModelSwarm003", "Estimate"]
    t_val <- sm$coefficients["ModelSwarm003", "t value"]
    p_val <- sm$coefficients["ModelSwarm003", "Pr(>|t|)"]
