...
# Set up local PSOCK cluster for Windows
cl <- makeCluster(detectCores() - 1)
clusterExport(cl, c("dat_clean", "lambda", "eval_baseline_5", "extract_baseline_5", "eval_mutant_manifold", "extract_mutant_manifold"))

set.seed(99)
dat_noise <- dat_clean %>% group_by(participant_idx) %>% mutate(`F` = sample(`F`)) %>% ungroup()
clusterExport(cl, c("dat_noise"))
rademacher_subjects <- 1:5 

cat("Phase 2: Baseline 5 True Data Fit...\n")
base_eval <- parLapply(cl, 1:S, function(s_idx) {
    library(dplyr)
...
base_noise_eval <- parLapply(cl, rademacher_subjects, function(s_idx) {
    library(dplyr)
...
for(var_idx in 1:10) {
    ...
    clusterExport(cl, "hyper")
    
    cand_eval <- parLapply(cl, 1:S, function(s_idx) {
        library(dplyr)
        d <- dat_clean %>% filter(participant_idx == s_idx)
        resp <- d$Boundary + 1; out <- d$`F`; rt <- d$RT
        obj <- function(phi) {
    ...
    cand_noise_eval <- parLapply(cl, rademacher_subjects, function(s_idx) {
        library(dplyr)
        d <- dat_noise %>% filter(participant_idx == s_idx)
        resp <- d$Boundary + 1; out <- d$`F`; rt <- d$RT
        obj <- function(phi) {
