...
// hyper: [0]lambda_sparse, [1]beta_ising, [2]K_sa, [3]theta_io, [4]lambda_MF
// phi: [0]a, [1]tnd, [2]v_ctx, [3]alpha_ctx, [4]gamma, [5]tau_omega, [6]lambda_sa

// [[Rcpp::export]]
double get_nll_swarm_003(const std::vector<double>& phi, const std::vector<double>& hyper,
    ...
    double theta_io = hyper[3], lambda_MF = hyper[4];
    ...
    for(int t=0;t<resp.size();++t){
        ...
        // 1. Feedforward Golgi High-Pass Gate
        double delta_Q = std::abs(alpha_ctx * prev_E); 
        double golgi_gate = 1.0 - std::exp(-lambda_MF * delta_Q);
        
        // 2. Spatio-Temporal Expansion & Trace
        for(int i=0;i<N;++i){
            frac_mem[i] = frac_alpha[i] * frac_mem[i] + (1.0 - frac_alpha[i]) * W_exp[i] * Q[ch];
            h[i] = std::tanh(frac_mem[i]) * golgi_gate; // Gated Expansion
            Z[i] = kappa_vec[i] * Z[i] + h[i];
        }
        
        // 3. IO Refractory Gating
        double e_io = std::abs(prev_E);
        bool io_spike = (e_io > theta_io) && ((t - last_spike) >= 1);
        ...
        // 5. Thermodynamics with Gap-Junction Ising Prior
