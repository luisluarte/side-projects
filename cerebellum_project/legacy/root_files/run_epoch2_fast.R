...
Rcpp::sourceCpp("src/models/epoch2_landscape.cpp")
...
genes_list <- list()
for(i in 1:10) { genes_list[[i]] <- c(1, 1, sample(0:1,1), sample(0:1,1)) }

fileConn <- file("magi_ledger.md", "a")
writeLines("\n## Epoch: 2 | 2026-08-25\n\n### 1. MAGI Consensus (Post-Epoch Deliberation)\n*   **Melchior:** Previous attempts focused on modulating processing speed (v) or bounds (a). But the cerebellum acts as a forward predictive model. It should bias the *starting point* ($w_t$) of the evidence accumulator. If certainty is high, it gives the cortex a head start.\n*   **Caspar:** This completely changes the density function geometry. A starting point shift mathematically decouples the expectation and variance in a non-linear way. I will enforce Gate 0.\n*   **Balthazar:** L2 defined: Topo 8 (Cerebellar Forward Predictive Bias). Base=Baseline5 (10 param), Cand=Topo8 (12 param).\n\n### 2. The Abstract Landscape Evaluated\n*   **Biological Topology:** Topology 8 (Cerebellar Forward Bias)\n*   **Equivalent Baseline:** Baseline 5 (Abstract Reward Integrator)\n\n### 3. Epoch Results Summary\n", fileConn)
close(fileConn)
...
for(var_idx in 1:10) {
    ...
    cand_eval <- mclapply(1:S, function(s_idx) {
        ...
        obj <- function(phi) {
            ll <- eval_topo_8(phi, genes, resp, out, rt, iti, f_dur)
            if(is.na(ll) || is.infinite(ll)) return(1e9)
            return(ll + lambda * sum(phi^2))
        }
        res <- optim(rep(0, 12), obj, method="L-BFGS-B", lower=rep(-5, 12), upper=rep(5, 12), control=list(maxit=30))
        rt_pred <- extract_topo_8(res$par, genes, resp, out, rt, iti, f_dur, FALSE)
        list(ll = res$value, rt_pred = rt_pred, trials = nrow(d), par=res$par)
    }, mc.cores = num_cores)
    ...
    obj1 <- function(phi) eval_topo_8(phi, genes, d1$Boundary+1, d1$`F`, d1$RT, d1$ITI, d1$F_dur)
    ...
    for(k in 1:20) {
        p_rand <- runif(12, -2, 2)
        rts <- extract_topo_8(p_rand, genes, d1$Boundary+1, d1$`F`, d1$RT, d1$ITI, d1$F_dur, FALSE)
    ...
    if(is_success && !is.na(Z_stat) && Z_stat < best_z) {
        best_z <- Z_stat
        best_name <- sprintf("MODEL_V%d_EPOCH2_BEST", var_idx)
    }
    
    cat(sprintf("Var %d: Rad_p=%.3f, F_Vol=%.1f, P_Ent=%.2f, Z=%.2f\n", var_idx, rad_p, fisher_vol, prior_ent, Z_stat))
}
