...
cat("Phase 2-4: Event-Weighted Grid Search on", num_cores, "cores...\n")
...
for (row in 1:nrow(grid_params)) {
    ...
    iter_res <- mclapply(1:S, function(s_idx) {
        ...
        obj_fg <- function(phi) { 
            ll_vec <- extract_ll_bvk_full_gating_dist_fatigue(phi, resp, out, rt, iti, f_dur, best_N)
            ll_vec[!is.finite(ll_vec)] <- -1e9
            return(-sum(W * ll_vec) + lambda * sum(phi^2))
        }
        ...
        pred_rt_fg <- extract_rt_bvk_full_gating_dist_fatigue(res_fg$par, resp, out, rt, iti, f_dur, best_N)
...
write_csv(final_results, "event_weighted_grid_results.csv")
cat("\n==========================================\n")
cat("EVENT-WEIGHTED OPTIMIZATION RESULTS\n")
cat("==========================================\n")
print(final_results)
