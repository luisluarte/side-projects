...
Rcpp::sourceCpp("magi_swarm_004.cpp")
...
cat("Phase 2: Evaluating Swarm 004...\n")
run_cand <- function(s_idx, h) {
    tryCatch({
        d <- d_list[[s_idx]]
        obj <- function(p){v<-get_nll_swarm_004(p,h,d$Boundary+1,d$Reward,d$RT);if(is.nan(v))1e6 else v}
        res <- tryCatch(cma_es(rep(0,7),obj,control=list(maxit=50,sigma=0.5)),error=function(e)list(par=rep(0,7),value=NA))
        data.frame(SubjectID=s_idx, NLL=res$value, Model="Swarm004")
    }, error=function(e) data.frame(SubjectID=s_idx, NLL=NA, Model="Swarm004"))
}

# h_swarm <- c(lambda_sparse, beta_ising, K_sa)
# Using CC_SPARSE_023 best params
...
df_all$Model <- factor(df_all$Model, levels=c("Base", "Swarm004"))

cat("\n========== STATISTICAL SUPREMACY TEST ==========\n")
if(nrow(df_all) > 10) {
    mod <- lmer(NLL ~ Model + (1 | SubjectID), data=df_all)
    sm <- summary(mod)
    est <- sm$coefficients["ModelSwarm004", "Estimate"]
    t_val <- sm$coefficients["ModelSwarm004", "t value"]
    p_val <- sm$coefficients["ModelSwarm004", "Pr(>|t|)"]
