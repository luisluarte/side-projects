pacman::p_load(tidyverse, doParallel, foreach, zoo, TTR, signal)
...
dat_clean <- dat_raw %>% arrange(participant_id, ttp) %>% 
    ...
    ungroup() %>% dplyr::filter(RT > 0.1, RT < 3.0, !is.na(Resp), !is.na(F)) %>%
    mutate(participant_idx = as.integer(as.factor(participant_id))) %>% dplyr::filter(participant_idx <= 30)

calc_probs <- function(s_idx) {
    d <- dat_clean %>% dplyr::filter(participant_idx == s_idx)
    n <- nrow(d)
...
calc_objectives <- function(df) {
    ...
    # 2. Welch's PSD Spectral Beta (1/f)
...
cat("Initializing 12-core cluster for Spectral Pareto Bootstraps...\n")
cl <- makeCluster(12)
registerDoParallel(cl)

set.seed(42)
boot_res <- foreach(b = 1:1000, .combine=rbind, .packages=c("tidyverse", "TTR", "signal")) %dopar% {
    sampled_subs <- sample(1:30, 30, replace=TRUE)
    boot_df <- bind_rows(lapply(1:30, function(i) {
        d <- sim_data %>% dplyr::filter(SubjectID == sampled_subs[i])
        d$SubjectID <- i 
        d
    }))
...
cat("=== SPECTRAL BOOTSTRAPPED PARETO STATISTICS (B=1000) ===\n")
...
write_csv(boot_df, "results/tables/magi_pareto_spectral_bootstrap_B1000.csv")
cat("SUCCESS: Spectral Bootstrapped Pareto sets saved.\n")
