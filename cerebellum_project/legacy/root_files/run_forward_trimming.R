...
cat("Starting Forward Trimming Strategy Shift Analysis (Removing Head) on", num_cores, "cores...\n")
...
results_list <- mclapply(1:S, function(s_idx) {
    ...
    for (H_val in H_seq) {
        ...
        subj_res[[length(subj_res) + 1]] <- data.frame(
            ...
            Trials_Retained = nrow(d_sub),
            Trials_Removed = H_val - 1,
            Delta_LL = delta_ll,
            Beta_RT = beta_rt
        )
    }
...
write_csv(final_df, "forward_trimming_results.csv")
...
p1 <- ggplot(final_df, aes(x = Trials_Removed, y = Delta_LL)) +
    ...
    labs(title = "Advantage of Dual Cognitive Model over Heuristic Base",
         x = "Trials Removed (from Start of Block)",
...
ggsave("forward_trimming_analysis_plot.pdf", p_combined, width = 8, height = 10)
ggsave("forward_trimming_analysis_plot.png", p_combined, width = 8, height = 10, dpi=300)
