...
Rcpp::sourceCpp("magi_swarm_001.cpp")
...
cat("Phase 2: Evaluating Swarm 001...\n")
run_cand <- function(s_idx, h) {
    tryCatch({
        d <- d_list[[s_idx]]
        obj <- function(p){v<-get_nll_swarm_001(p,h,d$Boundary+1,d$Reward,d$RT);if(is.nan(v))1e6 else v}
        res <- tryCatch(cma_es(rep(0,8),obj,control=list(maxit=50,sigma=0.5)),error=function(e)list(par=rep(0,8),value=NA))
        data.frame(SubjectID=s_idx, NLL=res$value, Model="Swarm001")
    }, error=function(e) data.frame(SubjectID=s_idx, NLL=NA, Model="Swarm001"))
}

# Hyperparameters based on previous success: k_fast=0.3, k_slow=0.95, lambda=1e-3, K_sa=18
h_swarm <- c(0.3, 0.95, 1e-3, 18)
df_cand <- bind_rows(mclapply(1:S, run_cand, h=h_swarm, mc.cores=CORES))

df_all <- bind_rows(df_base, df_cand) %>% drop_na()
df_all$Model <- factor(df_all$Model, levels=c("Base", "Swarm001"))

cat("\n========== STATISTICAL SUPREMACY TEST ==========\n")
if(nrow(df_all) > 10) {
    mod <- lmer(NLL ~ Model + (1 | SubjectID), data=df_all)
    sm <- summary(mod)
    est <- sm$coefficients["ModelSwarm001", "Estimate"]
    t_val <- sm$coefficients["ModelSwarm001", "t value"]
    p_val <- sm$coefficients["ModelSwarm001", "Pr(>|t|)"]
    
    # Cohen's d from lmer t-value: d = t / sqrt(N) for repeated measures
