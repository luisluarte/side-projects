...
Rcpp::sourceCpp("src/models/epoch9_qperturbed_wald.cpp") # for baseline_wald
...
writeLines("\n## Epoch: 10 (Multiplicative Q-Perturbed Wald | Variant 11.1)\n\n### 1. MAGI Consensus (DCN Gain Modulation & LASSO Bottleneck)\n*   **Caspar:** Unbounded additive perturbations allowed the RL predictor to memorize noise. Restoring the $L_1$ LASSO plasticity is mandatory to crush redundancy and protect the Rademacher capacity.\n*   **Balthazar:** The Deep Cerebellar Nuclei (DCN) do not overwrite the Cortex; they multiply it. $V_{eff} = Q_{ctx} \\cdot (1 + \\gamma \\tanh(Q_{cb}))$. Optimization via CMA-ES on the 9-dimensional space.\n", fileConn)
close(fileConn)

cat("Evaluating Multiplicative Fractional Manifold (Variant 11.1) with CMA-ES...\n")
...
cand_eval <- lapply(1:S, function(s_idx) {
    ...
    rt_pred <- extract_epoch10_multiplicative(res$par, hyper, resp, out, rt)
...
cat(sprintf("\n[FINAL CMA-ES RESULT] Sparse Multiplicative Wald: W1_Cand=%.4f, Beta_Cand=%.4f, Rad_p=%.3f\n", 
    total_cand_w1, total_cand_beta, rad_p))

fileConn <- file("magi_ledger.md", "a")
writeLines(sprintf("*   **Sparse Multiplicative Wald Result (N=30):** `W1_Cand=%.4f`, `Beta_Cand=%.4f`, `Rad_p=%.3f`", total_cand_w1, total_cand_beta, rad_p), fileConn)
close(fileConn)
cat("Epoch 10 Multiplicative Complete.\n")
