...
# phi_22: A, t_nd, beta_v, eta_LTP, eta_LTD, w_cb, lambda_shift, gamma_suppress, leak_lambda, mutual_omega
init_phi_22 <- c(log(2.0), log(0.3/0.7), log(3.0), log(0.5/0.5), log(0.5/0.5), 0.5, log(1.0), log(0.5), log(1.0), log(1.0))
...
for (p in participants) {
  ...
  ch_22 <- run_mcmc_subject(22, iters, init_phi_22, p_data$Resp, p_data$F, p_data$RT, p_data$delta_t)
  ll_22 <- extract_all_pointwise_ll(22, ch_22, p_data$Resp, p_data$F, p_data$RT, p_data$delta_t)
  ...
  r_eff_22 <- relative_eff(exp(ll_22), chain_id = rep(1, iters))
  loo_22 <- loo(ll_22, r_eff=r_eff_22)
  
  r_eff_6 <- relative_eff(exp(ll_6), chain_id = rep(1, iters))
  loo_6 <- loo(ll_6, r_eff=r_eff_6)
  
  res[[as.character(p)]] <- list(elpd_22 = loo_22$estimates["elpd_loo", "Estimate"], elpd_6 = loo_6$estimates["elpd_loo", "Estimate"])
}

elpd_22_total <- sum(sapply(res, function(x) x$elpd_22))
elpd_6_total <- sum(sapply(res, function(x) x$elpd_6))

sink("output_lca.txt")
cat(sprintf("Baseline (M6) elpd: %.2f\n", elpd_6_total))
cat(sprintf("LCA (M22) elpd: %.2f (Delta: %.2f)\n", elpd_22_total, elpd_22_total - elpd_6_total))
sink()
