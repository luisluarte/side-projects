...
Rcpp::sourceCpp("navarro_fuss_core.cpp")
...
run_nf <- function(s_idx) {
    tryCatch({
        d <- d_list[[s_idx]]
        
        # 1. Base Wald WFPT
        obj_base <- function(p) { v <- get_nll_base_nf(p, d$Boundary+1, d$Reward, d$RT); if(is.nan(v)) 1e6 else v }
        ...
        if(!is.na(res_base$value)) {
            e_bw <- ext_base_nf(res_base$par, d$Boundary+1, d$Reward, d$RT)
            mets_base <- calc_mets(e_bw[,1], d$Boundary) # wait, e_bw[,1] in C++ is index 0 (RT) and e_bw[,2] is index 1 (Prob).
        ...
        obj_rad_base <- function(p) { v <- get_nll_base_nf(p, R_noise+1, d$Reward, d$RT); if(is.nan(v)) 1e6 else v }
        res_rad_base <- tryCatch(cma_es(rep(0,4), obj_rad_base, control=list(maxit=30, sigma=0.5)), error=function(e) list(par=rep(0,4)))
        e_rad_base <- ext_base_nf(res_rad_base$par, R_noise+1, d$Reward, d$RT)
        rad_base <- abs(cor(e_rad_base[,2], R_noise))
        if(is.na(rad_base)) rad_base <- 0
        
        # 2. CC_Model_069 Hardcoded (h = c(1, 0.8, 20)) WFPT
        h_cand <- c(1, 0.8, 20)
        obj_cand <- function(p) { v <- get_nll_thermo_sudoku_nf(p, h_cand, d$Boundary+1, d$Reward, d$RT); if(is.nan(v)) 1e6 else v }
        ...
        if(!is.na(res_cand$value)) {
            e_cand <- ext_thermo_sudoku_nf(res_cand$par, h_cand, d$Boundary+1, d$Reward, d$RT)
            mets_cand <- calc_mets(e_cand[,2], d$Boundary)
            rmse_cand <- Metrics::rmse(d$RT, e_cand[,1])
        }
        
        obj_rad_cand <- function(p) { v <- get_nll_thermo_sudoku_nf(p, h_cand, R_noise+1, d$Reward, d$RT); if(is.nan(v)) 1e6 else v }
        res_rad_cand <- tryCatch(cma_es(rep(0, 7), obj_rad_cand, control=list(maxit=30, sigma=0.5)), error=function(e) list(par=rep(0,7)))
        e_rad_cand <- ext_thermo_sudoku_nf(res_rad_cand$par, h_cand, R_noise+1, d$Reward, d$RT)
...
cat("Evaluating WFPT (Navarro-Fuss) Corrected Models...\n")
df_res <- bind_rows(mclapply(1:S, run_nf, mc.cores=CORES)) %>% drop_na()

safe_wt <- function(x, y) { if(sd(x)==0 && sd(y)==0) return(1.0); tryCatch(wilcox.test(x, y, paired=TRUE)$p.value, error=function(e) 1.0) }

cat("\n======================================\n")
cat("CC_Model_069 (WFPT-Corrected) vs Baseline\n")
cat("======================================\n")
