...
// [[Rcpp::export]]
inline double eval_eccm_weighted_fit(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& delta_t) {
    ...
    // Calculate Inverse-Frequency Switch Weight
    int n_stay = 0;
    int n_switch = 0;
    for (int i=1; i<resp.size(); ++i) {
        if (resp[i] == resp[i-1]) n_stay++;
        else n_switch++;
    }
    double switch_weight = (n_switch > 0) ? ((double)n_stay / (double)n_switch) : 1.0;
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        double ll_penalty = -2.0 * std::log(wiener_pdf(rt[t], ch, safe_v, a, t_nd));
        
        // APPLY CLASS WEIGHTING
        if (t > 0 && resp[t] != resp[t-1]) {
            ll_penalty *= switch_weight;
        }
        
        D_vec.push_back(ll_penalty);
