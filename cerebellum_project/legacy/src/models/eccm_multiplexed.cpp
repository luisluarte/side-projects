...
// [[Rcpp::export]]
inline double eval_eccm_multiplexed(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& delta_t) {
    ...
    std::vector<double> W_PF_stay(N_GC, 0.0), W_PF_switch(N_GC, 0.0);
    std::vector<double> W_MLI_stay(N_MLI, 0.0), W_MLI_switch(N_MLI, 0.0);
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        int prev_ch = (t > 0) ? resp[t-1] : resp[0]; // Baseline assumption for t=0 is stay
        ...
        double V_stay = 0.0, V_switch = 0.0;
        for (int i=0; i<N_GC; ++i) { V_stay += W_PF_stay[i]*gc[i]; V_switch += W_PF_switch[i]*gc[i]; }
        for (int i=0; i<N_MLI; ++i) { V_stay -= W_MLI_stay[i]*mli[i]; V_switch -= W_MLI_switch[i]*mli[i]; }
        
        // Map Stay/Switch values back to Target 1 and Target 2
        double Q1_CB = (prev_ch == 1) ? V_stay : V_switch;
        double Q2_CB = (prev_ch == 2) ? V_stay : V_switch;
        ...
        bool is_stay = (t == 0) ? true : (ch == resp[t-1]);
        double err_cb = 0.0;
        
        if (is_stay) {
            err_cb = R_raw - V_stay;
            for (int i=0; i<N_GC; ++i) W_PF_stay[i] += lr_cb * err_cb * gc[i];
            for (int i=0; i<N_MLI; ++i) W_MLI_stay[i] -= lr_mli * err_cb * mli[i];
        } else {
            err_cb = R_raw - V_switch;
            for (int i=0; i<N_GC; ++i) W_PF_switch[i] += lr_cb * err_cb * gc[i];
            for (int i=0; i<N_MLI; ++i) W_MLI_switch[i] -= lr_mli * err_cb * mli[i];
        }
