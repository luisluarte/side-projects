...
// [[Rcpp::export]]
NumericVector extract_epoch9_qperturbed(const std::vector<double>& phi, const std::vector<double>& hyper, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt) {
    int N = 100; 
    ...
    double a_base = std::exp(phi[0]);
    double t_nd = 1.0 / (1.0 + std::exp(-phi[1])); // Static non-decision time
    double kappa_v = std::exp(phi[2]); 
    double alpha_ctx = 1.0 / (1.0 + std::exp(-phi[3])); 
    double alpha_cb = 1.0 / (1.0 + std::exp(-phi[4])); 
    double lambda_decay = 0.01 / (1.0 + std::exp(-phi[5])); // L2 Ridge decay [0, 0.01]
    double g_perturb = 2.0 * std::tanh(phi[6]); // Coupling strength [-2, 2]
    double scale_I = std::exp(phi[7]); 
    double gamma_spectral = std::exp(phi[8]); // Spectral scaling exponent
    ...
    for (int t = 0; t < T; ++t) {
        ...
        // II. Fractional Contextual Manifold
        for(int i=0; i<N; ++i) {
        ...
        // III. Cerebellar Value Prediction
        double Q_cb = 0.0;
        for(int i=0; i<N; ++i) {
            Q_cb += W_cb[i] * h[i];
        }
        
        // IV. Effective Value Hybridization
        double V_eff = Q_ctx[ch] + g_perturb * Q_cb;
        double v_t = kappa_v * std::max(V_eff, 0.01);
        
        // V. Pure Wald Generative Anchor
        double mu_wald = a_base / v_t;
        double lambda_wald = a_base * a_base;
        double t_cog = sample_wald(mu_wald, lambda_wald, gen, d_norm, d_unif);
        ...
        for(int i=0; i<N; ++i) {
            W_cb[i] = (1.0 - lambda_decay) * W_cb[i] + alpha_cb * RPE * h[i];
        }
        RPE_ctx_last = RPE;
