...
// [[Rcpp::export]]
inline double eval_eccm_hill(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& delta_t) {
    ...
    double theta = std::exp(phi[7]); 
    double hill_n = 1.0 + std::exp(phi[8]); // n >= 1
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        // HILL EQUATION DOUBLE WELL
        double s_factor = std::pow(theta, hill_n) / (std::pow(theta, hill_n) + std::pow(delta_cc, hill_n));
        double v_effective = v_t * s_factor;
