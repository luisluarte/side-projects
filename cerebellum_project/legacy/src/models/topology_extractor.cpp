...
// [[Rcpp::export]]
List extract_topology(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt) {
    ...
    NumericMatrix gc_mat(resp.size(), N_GC);
    NumericVector err_vec(resp.size());
    NumericVector q_ctx_vec(resp.size());
    NumericVector q_cb_vec(resp.size());
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        if (ch == 1) {
            err_cb = R_raw - Q1_CB;
            q_ctx_vec[t] = Q1_CTX; q_cb_vec[t] = Q1_CB;
            ...
            err_cb = R_raw - Q2_CB;
            q_ctx_vec[t] = Q2_CTX; q_cb_vec[t] = Q2_CB;
    ...
    return List::create(Named("GC") = gc_mat, Named("Error") = err_vec, Named("Q_CTX") = q_ctx_vec, Named("Q_CB") = q_cb_vec);
}
