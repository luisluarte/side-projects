...
// [[Rcpp::export]]
inline double eval_eccm_dynamic_boundary(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& delta_t) {
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        // DYNAMIC BOUNDARY & DRIFT MODULATION
        double v_effective = v_t * std::exp(-gamma_v * delta_cc);
        double a_effective = a * std::max(0.1, std::exp(-gamma_a * delta_cc)); // Floor at 10% of original boundary to prevent math errors
        
        double safe_v = std::abs(v_effective) < 1e-4 ? (v_effective >= 0 ? 1e-4 : -1e-4) : v_effective;
        D_vec.push_back(-2.0 * std::log(wiener_pdf(rt[t], ch, safe_v, a_effective, t_nd)));
