...
// [[Rcpp::export]]
List eval_metrics_eccm_dynamic_boundary(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& delta_t) {
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        double p1 = 1.0 / (1.0 + std::exp(-v_effective * a_effective));
        prob_ch1.push_back(p1);
        
        double ert = t_nd;
        if (std::abs(v_effective) < 1e-4) ert += (a_effective * a_effective) / 4.0;
        else ert += (a_effective / (2.0 * v_effective)) * std::tanh(v_effective * a_effective / 2.0);
