...
// [[Rcpp::export]]
List eval_metrics_eccm_entropy_reversal(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& delta_t) {
    ...
    double theta_rev = std::exp(phi[9]); 
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        double gain = 1.0 / (1.0 + theta_golgi * std::sqrt(mf_energy));
        ...
        double H_norm = entropy / std::log((double)N_GC); 
