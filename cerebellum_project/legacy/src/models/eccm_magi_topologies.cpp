...
// [[Rcpp::export]]
NumericVector extract_topology(const std::vector<double>& phi, const std::vector<int>& genes, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur, const NumericVector& ttp, bool return_ll) {
    ...
    for (int t = 0; t < T; ++t) {
        ...
        if (G3 == 0) {
            ...
            a_effective += beta_a * conflict + rho_ar * (prev_rt - prev_rt2);
        }
...
// [[Rcpp::export]]
inline double eval_magi_topo(const std::vector<double>& phi, const std::vector<int>& genes, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur, const NumericVector& ttp) {
    NumericVector ll = extract_topology(phi, genes, resp, out, rt, iti, f_dur, ttp, true);
    double nll = 0.0;
