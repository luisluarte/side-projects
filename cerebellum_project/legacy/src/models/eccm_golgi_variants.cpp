...
// [[Rcpp::export]]
inline double eval_eccm_golgi_ceiling(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& delta_t) {
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        // Dynamic Ceiling drops as MF energy rises
        ...
        for (int i=0; i<N_GC; ++i) {
            ...
            // Base ReLU, capped by the Golgi Ceiling to force uniformity/entropy!
...
// [[Rcpp::export]]
inline double eval_eccm_golgi_softmax(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& delta_t) {
    double a = std::exp(phi[0]), t_nd = 1.0 / (1.0 + std::exp(-phi[1])), beta_v = std::exp(phi[2]);
    double eta_LTP = 1.0 / (1.0 + std::exp(-phi[3])), eta_LTD = 1.0 / (1.0 + std::exp(-phi[4])), w_cb = phi[5]; 
    double lambda_shift = std::exp(phi[6]); 
    double gamma_suppress = std::exp(phi[7]); 
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        double mf_energy = 0.0;
        for (int j=0; j<N_MF; ++j) mf_energy += mf[j] * mf[j];
        // Temperature increases with MF energy
        ...
        for (int i=0; i<N_GC; ++i) gc[i] /= sum_exp; // Perfect Softmax Entropy Control!
