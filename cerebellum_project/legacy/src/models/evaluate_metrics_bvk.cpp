...
// [[Rcpp::export]]
List eval_metrics_eccm_bvk(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& delta_t) {
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        // BVK-specific computations
        double delta_CC = std::abs((Q1_CTX - Q2_CTX) - (Q1_CB - Q2_CB));
        
        // Starting point bias from cerebellum
        double w_raw = 0.5 + theta_cb * (Q1_CB - Q2_CB);
        double w_t = std::max(0.05, std::min(0.95, w_raw));
        
        // Drift suppression
        double v_base = beta_v * (Q1_CTX - Q2_CTX);
        double v_effective = v_base * std::exp(-kappa * delta_CC);
        
        // Choice probability: incorporate starting point bias
        // P(choice 1) from the Wiener model with bias w_t
        double p1 = 1.0 / (1.0 + std::exp(-v_effective * a * (2.0 * w_t - 1.0) - v_effective * a));
        // More accurate: use the analytical formula for choice probability with bias
        // P(upper) = [1 - exp(-2*v*a*w)] / [1 - exp(-2*v*a)]
        if (std::abs(v_effective) > 1e-4) {
            double e2vaw = std::exp(-2.0 * v_effective * a * w_t);
            double e2va = std::exp(-2.0 * v_effective * a);
            p1 = (1.0 - e2vaw) / (1.0 - e2va);
            p1 = std::max(1e-6, std::min(1.0 - 1e-6, p1));
        } else {
            p1 = w_t;  // When drift is near zero, choice probability equals starting point
        }
        prob_ch1.push_back(p1);
        
        // Expected RT with bias (mean first passage time)
        double ert = t_nd;
        if (std::abs(v_effective) < 1e-4) {
            ert += a * a * w_t * (1.0 - w_t);
        } else {
            // E[T] = (a/(2v)) * [1 - 2w + (exp(-2vaw)-1)/(exp(-2va)-1) * 2]
            // Simplified approximation:
            ert += (a / (2.0 * v_effective)) * std::tanh(v_effective * a * w_t);
        }
        exp_rt.push_back(std::max(t_nd + 0.01, ert));
