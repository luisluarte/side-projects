...
// [[Rcpp::export]]
inline double eval_eccm_golgi(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& delta_t) {
    ...
    double theta_golgi = std::exp(phi[8]); // Strength of Golgi Cell Divisive Normalization
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        // Divisive Normalization (Shunting Inhibition)
        double golgi_inhibition = 1.0 + theta_golgi * std::sqrt(mf_energy);
        
        std::vector<double> gc(N_GC, 0.0), mli(N_MLI, 0.0);
        for (int i=0; i<N_GC; ++i) {
            double act = 0.0; for (int j=0; j<N_MF; ++j) act += W_MF_GC[i][j] * mf[j];
            // Apply Golgi Inhibition to prevent saturation and maintain entropy!
