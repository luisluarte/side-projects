...
// [[Rcpp::export]]
NumericVector extract_topology_hfv(const std::vector<double>& phi, const std::vector<int>& genes, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur, const NumericVector& ttp, bool return_ll) {
    
    int TOPOLOGY = genes[0]; 
    int G1 = genes[1]; 
    int G2 = genes[2]; 
    int G3 = genes[3]; 
    int G4 = genes[4]; 
    ...
    for (int t = 0; t < T; ++t) {
        ...
        if (TOPOLOGY == 2) {
            U_1 = Q_ctx[0] * (1.0 + std::abs(Q_cb_1));
            U_2 = Q_ctx[1] * (1.0 + std::abs(Q_cb_2));
        }
        ...
        if (G1 == 0) {
            ...
            conflict = std::abs(Q_ctx[1] - Q_cb_2) + std::abs(Q_ctx[0] - Q_cb_1); 
        }
        ...
        if (G3 == 0) {
            ...
            a_effective += beta_vol_a * Omega_t;
        } else if (G3 == 2) { // The old AR hack
            double prev_rt = (t == 0) ? 0.5 : rt[t-1];
            double prev_rt2 = (t < 2) ? 0.5 : rt[t-2];
            a_effective += beta_a * conflict + (phi[15]) * (prev_rt - prev_rt2);
        } else if (G3 == 3) { // NEW HFV INJECTOR (Instantaneous Shock)
...
// [[Rcpp::export]]
inline double eval_magi_topo_hfv(const std::vector<double>& phi, const std::vector<int>& genes, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur, const NumericVector& ttp) {
    NumericVector ll = extract_topology_hfv(phi, genes, resp, out, rt, iti, f_dur, ttp, true);
    double nll = 0.0;
