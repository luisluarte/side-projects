...
// [[Rcpp::export]]
NumericVector extract_epoch7_corticopontine(const std::vector<double>& phi, const std::vector<double>& hyper, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt) {
    int N = 100; 
    ...
    double gamma_ema = 1.0 / (1.0 + std::exp(-phi[9])); // EMA decay rate
    ...
    // Fixed Mossy Fiber Projection Matrix (N x 5 orthogonal expansion)
    std::mt19937 init_gen(12345);
    std::normal_distribution<> d_w(0.0, 1.0);
    std::vector<std::vector<double>> W_MF(N, std::vector<double>(5));
    for(int i=0; i<N; ++i) {
        for(int j=0; j<5; ++j) {
    ...
    for (int t = 0; t < T; ++t) {
        ...
        // III. Cortico-Pontine Manifold (Hybrid Drive)
        double L1_norm = 0.0;
        for(int i=0; i<N; ++i) {
            // Expanded Multi-Modal Projection (5 Dimensions)
            double u_i = W_MF[i][0] * prev_choice + 
                         W_MF[i][1] * prev_rt + 
                         W_MF[i][2] * Q_ctx[0] + 
                         W_MF[i][3] * Q_ctx[1] + 
                         W_MF[i][4] * ema_delta;
        ...
        prev_rt = rt[t]; 
        ema_delta = (1.0 - gamma_ema) * ema_delta + gamma_ema * std::abs(RPE);
    }
    return rt_sim;
}
