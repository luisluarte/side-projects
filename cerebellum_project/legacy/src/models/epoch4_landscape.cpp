...
// [[Rcpp::export]]
NumericVector extract_topo_10(const std::vector<double>& phi, const std::vector<int>& genes, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur, bool return_ll) {
    // Topo 10: Cerebellar Motor-Gating (Non-Decision Time Bias)
    ...
    double t_nd_base = 1.0 / (1.0 + std::exp(-phi[4]));
    ...
    // Cerebellar Motor-Gate weights
    double beta_cb_tnd = 0.2 * (1.0 / (1.0 + std::exp(-phi[10]))); // Max reduction of 200ms
    ...
    for (int t = 0; t < T; ++t) {
        ...
        // Cortex and Striatum exclusively control the evidence accumulation integral
        double v_effective = kappa_v * delta_Q_ctx;
        if (genes[0] == 1) v_effective *= std::exp(beta_str_v * (0.5 - Str_bar)); 
        ...
        if (genes[1] == 1) a_effective += beta_str_a * (0.5 - Str_bar);
        
        if (a_effective < 0.01) a_effective = 0.01;
        if (a_effective > 10.0) a_effective = 10.0;
        
        // Epoch 4 Orthogonal Axis: Cerebellum acts strictly as a motor-gate, reducing Non-Decision Time (t_nd)
        double t_nd = t_nd_base;
        if (genes[2] == 1) {
            double certainty = std::abs(delta_Q_cb);
            t_nd = t_nd_base - beta_cb_tnd * std::tanh(certainty);
            if (t_nd < 0.1) t_nd = 0.1; // Biological minimum
        }
...
// [[Rcpp::export]]
inline double eval_topo_10(const std::vector<double>& phi, const std::vector<int>& genes, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur) {
    NumericVector ll = extract_topo_10(phi, genes, resp, out, rt, iti, f_dur, true);
    double nll = 0.0;
    for(int i=0; i<ll.size(); ++i) nll -= 2.0 * ll[i];
    return (std::isnan(nll) || std::isinf(nll)) ? 1e9 : nll;
}
