...
// [[Rcpp::export]]
NumericVector extract_topo_8(const std::vector<double>& phi, const std::vector<int>& genes, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur, bool return_ll) {
    // Topo 8: Cerebellar Forward Predictive Bias
    ...
    for (int t = 0; t < T; ++t) {
        ...
        // Cortex solely drives standard drift
        double v_effective = kappa_v * delta_Q_ctx;
        if(genes[0] == 1) v_effective *= std::exp(beta_str_v * (0.5 - Str_bar)); // Macroscopic fatigue on drift
        ...
        // Orthogonal Axis: Cerebellar Forward Model drives the Starting Point Bias (w_t)
        double bias_logit = 0.0;
        if(genes[2] == 1) bias_logit += theta_cb_bias * delta_Q_cb;
        if(genes[3] == 1) bias_logit += theta_ctx_bias * delta_Q_ctx;
        
        double w_bias = 0.5 + 0.45 * std::tanh(bias_logit);
...
// [[Rcpp::export]]
inline double eval_topo_8(const std::vector<double>& phi, const std::vector<int>& genes, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur) {
    NumericVector ll = extract_topo_8(phi, genes, resp, out, rt, iti, f_dur, true);
    double nll = 0.0;
    for(int i=0; i<ll.size(); ++i) nll -= 2.0 * ll[i];
    return (std::isnan(nll) || std::isinf(nll)) ? 1e9 : nll;
}
