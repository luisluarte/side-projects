...
// [[Rcpp::export]]
inline double eval_eccm_entropy_reversal(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& delta_t) {
    ...
    double theta_rev = std::exp(phi[9]); // New 10th parameter: Entropy Reversal Multiplier
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        double max_gc_abs = -1e9;
        for (int i=0; i<N_GC; ++i) {
            double act = 0.0; for (int j=0; j<N_MF; ++j) act += W_MF_GC[i][j] * mf[j];
            gc[i] = std::tanh(act * gain);
            if (std::abs(gc[i]) > max_gc_abs) max_gc_abs = std::abs(gc[i]);
        }
        
        // Compute Normalized Shannon Entropy of GC layer (using Softmax over absolute activations)
        double sum_exp = 0.0;
        std::vector<double> p_gc(N_GC, 0.0);
        for (int i=0; i<N_GC; ++i) {
            p_gc[i] = std::exp(std::abs(gc[i]) - max_gc_abs);
            sum_exp += p_gc[i];
        }
        double entropy = 0.0;
        for (int i=0; i<N_GC; ++i) {
            p_gc[i] /= sum_exp;
            if (p_gc[i] > 1e-12) entropy -= p_gc[i] * std::log(p_gc[i]);
        }
        double H_norm = entropy / std::log((double)N_GC); // [0, 1]
        ...
        // THE ENTROPY REVERSAL READOUT
        // If H_norm is high, (1 - theta_rev * H_norm) can become NEGATIVE, actively inverting the drift!
        double reversal_multiplier = 1.0 - (theta_rev * H_norm);
        double v_effective = v_t * std::exp(-gamma_suppress * delta_cc) * reversal_multiplier;
