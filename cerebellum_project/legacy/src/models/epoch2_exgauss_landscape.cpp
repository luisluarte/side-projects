...
// EX-GAUSSIAN LIKELIHOOD EVALUATOR
inline double log_exgauss_density(double RT, double tau, double mu, double sigma) {
    if (sigma < 1e-4) {
        if (RT > mu) return std::log(1.0 / tau) - ((RT - mu) / tau);
        return -20.72; // log(1e-9)
    }
    double arg1 = (mu / tau) + (sigma * sigma) / (2.0 * tau * tau) - (RT / tau);
    double arg2 = (RT - mu - (sigma * sigma) / tau) / sigma;
    
    double log_phi = R::pnorm(arg2, 0.0, 1.0, 1, 1);
    double log_p = std::log(1.0 / tau) + arg1 + log_phi;
    
    if (std::isnan(log_p) || std::isinf(log_p) || log_p < -20.72) return -20.72;
    return log_p;
}

// [[Rcpp::export]]
NumericVector extract_epoch2_exgauss(const std::vector<double>& phi, const std::vector<double>& hyper, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, bool return_ll) {
    int N = 100;
    ...
    for (int t = 0; t < T; ++t) {
        ...
        // I. Cortical Exponential Rate (Decision Time)
        ...
        // III. Stochastic Manifold Update & L1 Norm
        double L1_norm = 0.0;
        for(int i=0; i<N; ++i) {
        ...
        // V. Closed-Form Likelihood Evaluator
        if (return_ll) {
            out_vec[t] = log_exgauss_density(rt[t], tau, mu_tnd, sigma_tnd);
        } else {
            // Expected RT is mu + tau for Ex-Gaussian
            out_vec[t] = mu_tnd + tau;
        }
...
// [[Rcpp::export]]
inline double eval_epoch2_exgauss(const std::vector<double>& phi, const std::vector<double>& hyper, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt) {
    NumericVector ll = extract_epoch2_exgauss(phi, hyper, resp, out, rt, true);
    double nll = 0.0;
    for(int i=0; i<ll.size(); ++i) nll -= 2.0 * ll[i];
    return (std::isnan(nll) || std::isinf(nll)) ? 1e9 : nll;
}
