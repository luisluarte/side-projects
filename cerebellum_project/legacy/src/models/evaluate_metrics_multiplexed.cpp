...
// [[Rcpp::export]]
List eval_metrics_eccm_multiplexed(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& delta_t) {
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        int prev_ch = (t > 0) ? resp[t-1] : resp[0];
