...
inline double calc_expected_rt_dist(double v, double a, double w, double t_nd) {
    double z = w * a;
    if (std::abs(v) < 1e-4) {
        return t_nd + z * (a - z);
    } else {
...
// [[Rcpp::export]]
inline double eval_bvk_full_gating_dist_fatigue(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur, int N_MF) {
...
// [[Rcpp::export]]
NumericVector extract_rt_bvk_full_gating_dist_fatigue(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur, int N_MF) {
    double alpha_ctx = 1.0 / (1.0 + std::exp(-phi[0]));
    double eta_gc = 0.5 + std::exp(phi[1]);
    double lambda_gc = std::exp(phi[2]);
    double theta_cb = std::exp(phi[3]);
    ...
    for (int t = 0; t < T; ++t) {
        ...
        pred_rt[t] = calc_expected_rt_dist(safe_v, a_effective, w_bias, tau_nd);
...
// [[Rcpp::export]]
NumericVector extract_ll_bvk_full_gating_dist_fatigue(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur, int N_MF) {
    double alpha_ctx = 1.0 / (1.0 + std::exp(-phi[0]));
    double eta_gc = 0.5 + std::exp(phi[1]);
    double lambda_gc = std::exp(phi[2]);
    double theta_cb = std::exp(phi[3]);
