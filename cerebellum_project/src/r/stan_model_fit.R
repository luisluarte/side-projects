  ...
  for(t in 1:n) {
    ...
    if(t > 1) {
      ...
      veff_v <- v_vctx[s] * (Q_switch_v - Q_stay_v)
        q_diff_v[t] <- Q_v[2] - Q_v[1]
      if (is.na(veff_v) || veff_v == 0) {
      ...
      veff_m <- m_vctx[s] * (Q_switch_m - Q_stay_m)
        q_diff_m[t] <- Q_m[2] - Q_m[1]
        z_diff_m[t] <- Z[2] - Z[1]
      if (is.na(veff_m) || veff_m == 0) {
...
sink("/home/DCCS5/cerebellum_project/results/conf_ss3_N30.txt")
...
saveRDS(df_res, '/home/DCCS5/cerebellum_project/results/df_res_ss3_N30.rds')
