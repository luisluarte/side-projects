...
List evaluate_ncra_eccm_cv(
  ...
  double kappa_nc = phi_18d[17]; // 18th parameter: Nucleo-Cortical Feedback Gain
  ...
  for(int j=0; j<N_MF; ++j) {
      mf_c[j] = std::floor(rng.runif() * 7.0); // 7 Dimensions (6 inputs + 1 feedback)
  ...
  std::vector<std::vector<double>> state_hist(15, std::vector<double>(7, 0.0));
  ...
  for (int s = 0; s < num_participants; ++s) {
    ...
    for (int t=0; t<N_t; ++t) {
        ...
        state_hist[0][6] = std::tanh(kappa_nc * D_t); // Nucleo-Cortical Feedback
...
lower_ncra <- c(lower_eccm, 0.0)
upper_ncra <- c(upper_eccm, 5.0)
init_ncra <- c(init_eccm, 1.0)

cat("\nTraining NCRA ECCM (18 parameters)\n")
obj_ncra <- function(phi) {
    if (any(phi < lower_ncra) || any(phi > upper_ncra)) return(1e9)
    res <- evaluate_ncra_eccm_cv(as.numeric(phi), as.integer(dat_all$Resp), as.integer(dat_all$F), as.numeric(dat_all$Bd1), as.numeric(dat_all$Bd2), as.numeric(dat_all$RT), as.numeric(dat_all$ttp), as.numeric(dat_all$ttF), as.integer(dat_all$participant_factor), as.integer(dat_all$is_test), as.integer(20), as.integer(200), as.integer(40), num_participants)
    return(res$test_nlls[1]) 
}
cma_ncra <- cma_es(init_ncra, obj_ncra, lower = lower_ncra, upper = upper_ncra, control = list(maxit = 35, trace = FALSE, sigma = 0.2))
res_ncra <- evaluate_ncra_eccm_cv(as.numeric(cma_ncra$par), as.integer(dat_all$Resp), as.integer(dat_all$F), as.numeric(dat_all$Bd1), as.numeric(dat_all$Bd2), as.numeric(dat_all$RT), as.numeric(dat_all$ttp), as.numeric(dat_all$ttF), as.integer(dat_all$participant_factor), as.integer(dat_all$is_test), as.integer(20), as.integer(200), as.integer(40), num_participants)

ncra_test_nlls <- res_ncra$test_nlls[2:(num_participants+1)]
ncra_metrics <- calc_metrics(res_ncra)
...
cat(sprintf("RT RMSE : %.3f\n", ncra_metrics$RT_RMSE))
cat(sprintf("Accuracy: %.3f\n", ncra_metrics$Accuracy))
cat("--------------------------------------\n")
...
results_df <- data.frame(
    ...
    NCRA_ECCM_NLL = ncra_test_nlls
)
write.csv(results_df, "results/tables/ncra_biological_benchmark.csv", row.names=FALSE)

cat("\n--- PAIRED T-TESTS (Participant Level) ---\n")
t1 <- t.test(pure_eccm_test_nlls, ncra_test_nlls, paired=TRUE)
cat(sprintf("Pure ECCM vs NCRA ECCM: t = %.2f, p = %.3e (Mean Diff: %.2f)\n", t1$statistic, t1$p.value, t1$estimate))

t2 <- t.test(m1_test_nlls, ncra_test_nlls, paired=TRUE)
cat(sprintf("M1 (WSLS) vs NCRA ECCM: t = %.2f, p = %.3e (Mean Diff: %.2f)\n", t2$statistic, t2$p.value, t2$estimate))
