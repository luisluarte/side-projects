...
List evaluate_mixed_eccm_cv(
    ...
    const IntegerVector& is_test_R, int N_MF, int N_GC, int N_MLI, bool use_mixed, int num_participants
) {
  int N_t = resp_R.size();
  ...
  double beta_v_wsls = use_mixed ? phi_18d[17] : 0.0; // 18th parameter
  ...
  for (int t=0; t<N_t; ++t) {
    ...
    double v_t_ddm = beta_v_eccm * D_t + beta_v_wsls * wsls_signal;
...
res_eccm <- evaluate_mixed_eccm_cv(as.numeric(cma_eccm$par), as.integer(dat_all$Resp), as.integer(dat_all$F), as.numeric(dat_all$Bd1), as.numeric(dat_all$Bd2), as.numeric(dat_all$RT), as.numeric(dat_all$ttp), as.numeric(dat_all$ttF), as.integer(dat_all$participant_factor), as.integer(dat_all$is_test), as.integer(20), as.integer(200), as.integer(40), FALSE, num_participants)

eccm_test_nlls <- res_eccm$test_nlls[2:(num_participants+1)]
eccm_metrics <- calc_metrics(res_eccm)

cat("\nTraining Mixed ECCM+WSLS (18 parameters)\n")
lower_mixed <- lower_eccm
upper_mixed <- upper_eccm
upper_mixed[18] <- 5.0 # allow b_wsls up to 5.0
init_mixed <- lower_mixed + (upper_mixed - lower_mixed) / 2

obj_mixed <- function(phi) {
    if (any(phi < lower_mixed) || any(phi > upper_mixed)) return(1e9)
    res <- evaluate_mixed_eccm_cv(as.numeric(phi), as.integer(dat_all$Resp), as.integer(dat_all$F), as.numeric(dat_all$Bd1), as.numeric(dat_all$Bd2), as.numeric(dat_all$RT), as.numeric(dat_all$ttp), as.numeric(dat_all$ttF), as.integer(dat_all$participant_factor), as.integer(dat_all$is_test), as.integer(20), as.integer(200), as.integer(40), TRUE, num_participants)
    return(res$test_nlls[1]) 
}
cma_mixed <- cma_es(init_mixed, obj_mixed, lower = lower_mixed, upper = upper_mixed, control = list(maxit = 35, trace = FALSE, sigma = 0.2))
res_mixed <- evaluate_mixed_eccm_cv(as.numeric(cma_mixed$par), as.integer(dat_all$Resp), as.integer(dat_all$F), as.numeric(dat_all$Bd1), as.numeric(dat_all$Bd2), as.numeric(dat_all$RT), as.numeric(dat_all$ttp), as.numeric(dat_all$ttF), as.integer(dat_all$participant_factor), as.integer(dat_all$is_test), as.integer(20), as.integer(200), as.integer(40), TRUE, num_participants)

mixed_test_nlls <- res_mixed$test_nlls[2:(num_participants+1)]
mixed_metrics <- calc_metrics(res_mixed)
...
cat("MIXED ECCM+WSLS\n")
cat(sprintf("Test NLL: %.2f\n", sum(mixed_test_nlls)))
cat(sprintf("PR-AUC  : %.3f\n", mixed_metrics$PR_AUC))
cat(sprintf("RT RMSE : %.3f\n", mixed_metrics$RMSE))
cat(sprintf("Accuracy: %.3f\n", mixed_metrics$Accuracy))
cat("--------------------------------------\n")
...
results_df <- data.frame(
    ...
    M1_NLL = m1_test_nlls,
    Pure_ECCM_NLL = eccm_test_nlls,
    Mixed_ECCM_NLL = mixed_test_nlls
)
write.csv(results_df, "results/tables/final_128_mixed_benchmark.csv", row.names=FALSE)

cat("\n--- PAIRED T-TESTS (Participant Level) ---\n")
t1 <- t.test(eccm_test_nlls, mixed_test_nlls, paired=TRUE)
cat(sprintf("Pure ECCM vs Mixed ECCM+WSLS: t = %.2f, p = %.3e (Mean Diff: %.2f)\n", t1$statistic, t1$p.value, t1$estimate))

cat(sprintf("M1 (WSLS) Aggregate NLL: %.2f\n", sum(m1_test_nlls)))

t2 <- t.test(m1_test_nlls, mixed_test_nlls, paired=TRUE)
cat(sprintf("M1 (WSLS) vs Mixed ECCM+WSLS: t = %.2f, p = %.3e (Mean Diff: %.2f)\n", t2$statistic, t2$p.value, t2$estimate))
