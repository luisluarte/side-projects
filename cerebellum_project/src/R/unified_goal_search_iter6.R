...
cat("Loading dataset Iteration 6...\n")
...
// Arch P: Parameterized Modulatory Cerebellum
// Cortex and Cerebellum learn independently. Cerebellum modulates cortex by a fitted weight.
// [[Rcpp::export]]
List eval_arch_p(const NumericVector& phi, const IntegerVector& resp_R, const IntegerVector& out_R, const IntegerVector& subj_idx_R, const NumericVector& rt_R) {
...
// Arch O: Threshold Modulation
// Cerebellum predicts magnitude of cortical error, modulates decision threshold "a".
// [[Rcpp::export]]
List eval_arch_o(const NumericVector& phi, const IntegerVector& resp_R, const IntegerVector& out_R, const IntegerVector& subj_idx_R, const NumericVector& rt_R) {
  double a_base = phi[0]; double t_nd = phi[1]; double beta_v = phi[2]; double eta_LTP = phi[3]; double eta_LTD = phi[4]; double w_cb = phi[5];
  double nll = 0.0; std::vector<double> out_prob1; 
  ...
  for (int t=0; t<resp_R.size(); ++t) {
    ...
    Q_CB = std::max(0.0, std::min(2.0, Q_CB));
    
    double a_t = std::max(0.1, a_base + w_cb * Q_CB);
    double v_t = beta_v * ( Q1_CTX - Q2_CTX );
    double safe_v_t = std::abs(v_t) < 1e-4 ? (v_t >= 0 ? 1e-4 : -1e-4) : v_t;
    nll -= std::log(wiener_pdf(rt_R[t], ch, safe_v_t, a_t, t_nd));
...
cat("Optimizing Architecture P: Parameterized Modulatory Cerebellum...\n")
opt_p <- cma_es(c(1.0, 0.3, 2.0, 0.5, 0.5, 0.5), 
                function(p) { if(any(p<c(0.1,0.01,0,0,0,-5.0))||any(p>c(5,1,10,1,1,5.0))) 1e9 else eval_arch_p(p, dat_all$Resp, dat_all$F, dat_all$participant_factor, dat_all$RT)$nll }, 
                lower=c(0.1, 0.01, 0, 0, 0, -5.0), upper=c(5, 1, 10, 1, 1, 5.0), control=list(maxit=50, trace=FALSE))

cat("Optimizing Architecture O: Cerebellar-Modulated Threshold...\n")
opt_o <- cma_es(c(1.0, 0.3, 2.0, 0.5, 0.5, 0.5), 
                function(p) { if(any(p<c(0.1,0.01,0,0,0,0.0))||any(p>c(5,1,10,1,1,5.0))) 1e9 else eval_arch_o(p, dat_all$Resp, dat_all$F, dat_all$participant_factor, dat_all$RT)$nll }, 
...
res_p <- eval_arch_p(opt_p$par, dat_all$Resp, dat_all$F, dat_all$participant_factor, dat_all$RT)
res_o <- eval_arch_o(opt_o$par, dat_all$Resp, dat_all$F, dat_all$participant_factor, dat_all$RT)

dat_all$p1_cfmr <- res_cfmr$prob1
dat_all$p1_p <- res_p$prob1
dat_all$p1_o <- res_o$prob1
...
subj_res <- dat_all %>% group_by(participant_id) %>% summarize(
    ...
    nll_p = -sum(log(pmax(1e-15, pmin(1 - 1e-15, ifelse(Resp==1, p1_p, 1-p1_p))))),
    nll_o = -sum(log(pmax(1e-15, pmin(1 - 1e-15, ifelse(Resp==1, p1_o, 1-p1_o))))),
    prauc_cfmr = pr.curve(scores.class0 = p1_cfmr, weights.class0 = ifelse(Resp==1, 1, 0))$auc.integral,
    prauc_p = pr.curve(scores.class0 = p1_p, weights.class0 = ifelse(Resp==1, 1, 0))$auc.integral,
    prauc_o = pr.curve(scores.class0 = p1_o, weights.class0 = ifelse(Resp==1, 1, 0))$auc.integral
)

cat("\n=======================================================\n")
cat("          UNIFIED MODEL SEARCH ITERATION 6             \n")
cat("=======================================================\n")
...
tt_nll_p <- safe_t_test(subj_res$nll_cfmr, subj_res$nll_p, "greater") 
tt_nll_o <- safe_t_test(subj_res$nll_cfmr, subj_res$nll_o, "greater")

tt_pr_p <- safe_t_test(subj_res$prauc_p, subj_res$prauc_cfmr, "greater") 
tt_pr_o <- safe_t_test(subj_res$prauc_o, subj_res$prauc_cfmr, "greater")

res_df <- data.frame(
    Model = c("Arch P: Modulatory Parameterized", "Arch O: Threshold Modulation"),
    Total_NLL = c(sum(subj_res$nll_p), sum(subj_res$nll_o)),
    Mean_PRAUC = c(mean(subj_res$prauc_p, na.rm=TRUE), mean(subj_res$prauc_o, na.rm=TRUE)),
    NLL_PValue = c(tt_nll_p$p.value, tt_nll_o$p.value),
    PRAUC_PValue = c(tt_pr_p$p.value, tt_pr_o$p.value)
)
