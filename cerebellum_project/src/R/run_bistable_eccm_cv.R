...
// [[Rcpp::export]]
List evaluate_bistable_eccm_cv(
  ...
  double rho_bistable = phi_18d[17]; // 18th parameter
  ...
  for (int s = 0; s < num_participants; ++s) {
    ...
    for (int t=0; t<N_t; ++t) {
        ...
        if (t > 0 && subj_idx_R[t-1] == s_idx && out_R[t-1] == 0) {
            B_t = B_t * rho_bistable;
        }
        ...
        double pc_diff = (y_PC1 * B_t) - (y_PC2 * B_t);
        D_t = (1.0 - alpha_dcn) * D_t + alpha_dcn * pc_diff; 
        
        double a_t = std::max(0.30, a_0 + kappa_a*norm_S);
        double dens = wiener_pdf(rt_R[t], ch, beta_v_eccm * D_t, a_t, t_nd);
        ...
        if (is_test_R[t] == 0) {
            ...
            for (int k=0; k<N_MLI; ++k) {
                W_PF1[k] += eta*((ch==1?target_chosen:target_unchosen) - y_PC1)*e_trace[k] - lambda*W_PF1[k]; 
                W_PF2[k] += eta*((ch==2?target_chosen:target_unchosen) - y_PC2)*e_trace[k] - lambda*W_PF2[k]; 
            }
        } else {
            test_nlls[s+1] += nll_t;
            out_prob1.push_back(1.0 / (1.0 + std::exp(-a_t * (beta_v_eccm * D_t))));
            out_pred_rt.push_back(t_nd + (a_t / (2.0 * (beta_v_eccm * D_t))) * std::tanh(a_t * (beta_v_eccm * D_t) / 2.0));
...
lower_bistable <- c(lower_eccm, 0.0)
upper_bistable <- c(upper_eccm, 2.0)
init_bistable <- c(init_eccm, 1.0)

cat("\nTraining Bistable ECCM (18 parameters)\n")
obj_bistable <- function(phi) {
    if (any(phi < lower_bistable) || any(phi > upper_bistable)) return(1e9)
    res <- evaluate_bistable_eccm_cv(as.numeric(phi), as.integer(dat_all$Resp), as.integer(dat_all$F), as.numeric(dat_all$Bd1), as.numeric(dat_all$Bd2), as.numeric(dat_all$RT), as.numeric(dat_all$ttp), as.numeric(dat_all$ttF), as.integer(dat_all$participant_factor), as.integer(dat_all$is_test), as.integer(20), as.integer(200), as.integer(40), num_participants)
    return(res$test_nlls[1]) 
}
cma_bistable <- cma_es(init_bistable, obj_bistable, lower = lower_bistable, upper = upper_bistable, control = list(maxit = 35, trace = FALSE, sigma = 0.2))
res_bistable <- evaluate_bistable_eccm_cv(as.numeric(cma_bistable$par), as.integer(dat_all$Resp), as.integer(dat_all$F), as.numeric(dat_all$Bd1), as.numeric(dat_all$Bd2), as.numeric(dat_all$RT), as.numeric(dat_all$ttp), as.numeric(dat_all$ttF), as.integer(dat_all$participant_factor), as.integer(dat_all$is_test), as.integer(20), as.integer(200), as.integer(40), num_participants)

bistable_test_nlls <- res_bistable$test_nlls[2:(num_participants+1)]
bistable_metrics <- calc_metrics(res_bistable)

cat("\n======================================\n")
cat("AGGREGATE RESULTS ACROSS 30 PARTICIPANTS\n")
cat("--------------------------------------\n")
cat("BISTABLE ECCM\n")
cat(sprintf("Test NLL: %.2f\n", sum(bistable_test_nlls)))
cat(sprintf("PR-AUC  : %.3f\n", bistable_metrics$PRAUC))
cat(sprintf("RT RMSE : %.3f\n", bistable_metrics$RT_RMSE))
cat(sprintf("Accuracy: %.3f\n", bistable_metrics$Accuracy))
cat("--------------------------------------\n")
...
results_df <- data.frame(
    ...
    Bistable_ECCM_NLL = bistable_test_nlls
)
write.csv(results_df, "results/tables/bistable_biological_benchmark.csv", row.names=FALSE)

cat("\n--- PAIRED T-TESTS (Participant Level) ---\n")
t1 <- t.test(pure_eccm_test_nlls, bistable_test_nlls, paired=TRUE)
cat(sprintf("Pure ECCM vs Bistable ECCM: t = %.2f, p = %.3e (Mean Diff: %.2f)\n", t1$statistic, t1$p.value, t1$estimate))

t2 <- t.test(m1_test_nlls, bistable_test_nlls, paired=TRUE)
cat(sprintf("M1 (WSLS) vs Bistable ECCM: t = %.2f, p = %.3e (Mean Diff: %.2f)\n", t2$statistic, t2$p.value, t2$estimate))
