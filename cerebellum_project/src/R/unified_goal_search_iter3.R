...
cat("Loading dataset Iteration 3...\n")
...
// Arch G: Residual Correction + Weight Decay (Regularized Cerebellum)
// [[Rcpp::export]]
List eval_arch_g(const NumericVector& phi, const IntegerVector& resp_R, const IntegerVector& out_R, const IntegerVector& subj_idx_R, const NumericVector& rt_R) {
  double a = phi[0]; double t_nd = phi[1]; double beta_v = phi[2]; double eta_LTP = phi[3]; double eta_LTD = phi[4]; double decay = phi[5];
  double nll = 0.0; std::vector<double> out_prob1; 
  ...
  for (int t=0; t<resp_R.size(); ++t) {
    ...
    if (ch == 1) {
        ...
        for (int i=0; i<N_GC; ++i) W_PF1[i] = (1.0 - decay)*W_PF1[i] + lr_cb * delta_cb * gc[i];
        for (int i=0; i<N_MLI; ++i) W_MLI1[i] = (1.0 - decay)*W_MLI1[i] - lr_mli * delta_cb * mli[i];
    } else {
        double delta_ctx = R_raw - Q2_CTX;
        Q2_CTX += (out == 1) ? eta_LTP * (1.0 - Q2_CTX) : eta_LTD * (-1.0 - Q2_CTX);
        double delta_cb = delta_ctx - Q2_CB;
        for (int i=0; i<N_GC; ++i) W_PF2[i] = (1.0 - decay)*W_PF2[i] + lr_cb * delta_cb * gc[i];
        for (int i=0; i<N_MLI; ++i) W_MLI2[i] = (1.0 - decay)*W_MLI2[i] - lr_mli * delta_cb * mli[i];
    }
...
// Arch H: Modulatory Cerebellum (Multiplicative)
// Cortex is amplified or suppressed by Cerebellum: v_t = Q_CTX * (1 + Q_CB)
// [[Rcpp::export]]
List eval_arch_h(const NumericVector& phi, const IntegerVector& resp_R, const IntegerVector& out_R, const IntegerVector& subj_idx_R, const NumericVector& rt_R) {
  double a = phi[0]; double t_nd = phi[1]; double beta_v = phi[2]; double eta_LTP = phi[3]; double eta_LTD = phi[4]; double decay = phi[5];
  double nll = 0.0; std::vector<double> out_prob1; 
  ...
  for (int t=0; t<resp_R.size(); ++t) {
    ...
    double v_t = beta_v * ( (Q1_CTX * (1.0 + Q1_CB)) - (Q2_CTX * (1.0 + Q2_CB)) );
    double safe_v_t = std::abs(v_t) < 1e-4 ? (v_t >= 0 ? 1e-4 : -1e-4) : v_t;
    nll -= std::log(wiener_pdf(rt_R[t], ch, safe_v_t, a, t_nd));
    out_prob1.push_back(1.0 / (1.0 + std::exp(-a * safe_v_t)));
    ...
    if (ch == 1) {
        ...
        Q1_CTX += (out == 1) ? eta_LTP * (1.0 - Q1_CTX) : eta_LTD * (-1.0 - Q1_CTX);
        for (int i=0; i<N_GC; ++i) W_PF1[i] = (1.0 - decay)*W_PF1[i] + lr_cb * err_cb * gc[i];
        for (int i=0; i<N_MLI; ++i) W_MLI1[i] = (1.0 - decay)*W_MLI1[i] - lr_mli * err_cb * mli[i];
    } else {
        double err_cb = R_raw - Q2_CB;
        Q2_CTX += (out == 1) ? eta_LTP * (1.0 - Q2_CTX) : eta_LTD * (-1.0 - Q2_CTX);
        for (int i=0; i<N_GC; ++i) W_PF2[i] = (1.0 - decay)*W_PF2[i] + lr_cb * err_cb * gc[i];
        for (int i=0; i<N_MLI; ++i) W_MLI2[i] = (1.0 - decay)*W_MLI2[i] - lr_mli * err_cb * mli[i];
    }
...
cat("Optimizing Architecture G: Residual Correction with Decay...\n")
opt_g <- cma_es(c(1.0, 0.3, 2.0, 0.5, 0.5, 0.05), 
                function(p) { if(any(p<c(0.1,0.01,0,0,0,0.0))||any(p>c(5,1,10,1,1,0.5))) 1e9 else eval_arch_g(p, dat_all$Resp, dat_all$F, dat_all$participant_factor, dat_all$RT)$nll }, 
                lower=c(0.1, 0.01, 0, 0, 0, 0.0), upper=c(5, 1, 10, 1, 1, 0.5), control=list(maxit=50, trace=FALSE))

cat("Optimizing Architecture H: Modulatory Cerebellum...\n")
opt_h <- cma_es(c(1.0, 0.3, 2.0, 0.5, 0.5, 0.05), 
                function(p) { if(any(p<c(0.1,0.01,0,0,0,0.0))||any(p>c(5,1,10,1,1,0.5))) 1e9 else eval_arch_h(p, dat_all$Resp, dat_all$F, dat_all$participant_factor, dat_all$RT)$nll }, 
                lower=c(0.1, 0.01, 0, 0, 0, 0.0), upper=c(5, 1, 10, 1, 1, 0.5), control=list(maxit=50, trace=FALSE))

res_cfmr <- eval_cfmr(opt_cfmr$par, dat_all$Resp, dat_all$F, dat_all$participant_factor, dat_all$RT)
res_g <- eval_arch_g(opt_g$par, dat_all$Resp, dat_all$F, dat_all$participant_factor, dat_all$RT)
res_h <- eval_arch_h(opt_h$par, dat_all$Resp, dat_all$F, dat_all$participant_factor, dat_all$RT)

dat_all$p1_cfmr <- res_cfmr$prob1
dat_all$p1_g <- res_g$prob1
dat_all$p1_h <- res_h$prob1
...
subj_res <- dat_all %>% group_by(participant_id) %>% summarize(
    ...
    nll_g = -sum(log(pmax(1e-15, pmin(1 - 1e-15, ifelse(Resp==1, p1_g, 1-p1_g))))),
    nll_h = -sum(log(pmax(1e-15, pmin(1 - 1e-15, ifelse(Resp==1, p1_h, 1-p1_h))))),
    prauc_cfmr = pr.curve(scores.class0 = p1_cfmr, weights.class0 = ifelse(Resp==1, 1, 0))$auc.integral,
    prauc_g = pr.curve(scores.class0 = p1_g, weights.class0 = ifelse(Resp==1, 1, 0))$auc.integral,
    prauc_h = pr.curve(scores.class0 = p1_h, weights.class0 = ifelse(Resp==1, 1, 0))$auc.integral
)

cat("\n=======================================================\n")
cat("          UNIFIED MODEL SEARCH ITERATION 3             \n")
cat("=======================================================\n")
...
tt_nll_g <- safe_t_test(subj_res$nll_cfmr, subj_res$nll_g, "greater") 
tt_nll_h <- safe_t_test(subj_res$nll_cfmr, subj_res$nll_h, "greater")

tt_pr_g <- safe_t_test(subj_res$prauc_g, subj_res$prauc_cfmr, "greater") 
tt_pr_h <- safe_t_test(subj_res$prauc_h, subj_res$prauc_cfmr, "greater")

res_df <- data.frame(
    Model = c("Arch G: Residual + Decay", "Arch H: Modulatory Cerebellum"),
    Total_NLL = c(sum(subj_res$nll_g), sum(subj_res$nll_h)),
    Mean_PRAUC = c(mean(subj_res$prauc_g, na.rm=TRUE), mean(subj_res$prauc_h, na.rm=TRUE)),
    NLL_PValue = c(tt_nll_g$p.value, tt_nll_h$p.value),
    PRAUC_PValue = c(tt_pr_g$p.value, tt_pr_h$p.value)
)
