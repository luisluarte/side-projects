...
// [[Rcpp::export]]
List evaluate_saturation_eccm_cv(
  ...
  double gamma_dcn = phi_18d[17]; // 18th parameter: DCN Saturation Gain
  ...
  for(int j=0; j<N_MF; ++j) {
      mf_c[j] = std::floor(rng.runif() * 6.0); 
  ...
  for (int i=0; i<N_GC; ++i) {
      for (int k=0; k<4; ++k) {
          gc_mossy_map[i][k] = std::floor(rng.runif() * N_MF);
  ...
  for (int s = 0; s < num_participants; ++s) {
    ...
    for (int t=0; t<N_t; ++t) {
        ...
        x_t = (1.0 - alpha_dcn) * x_t + alpha_dcn * pc_diff; 
        
        double D_t = std::tanh(gamma_dcn * x_t);
...
lower_saturation <- c(lower_eccm, 0.1)
upper_saturation <- c(upper_eccm, 10.0)
init_saturation <- c(init_eccm, 5.0)

cat("\nTraining Saturation ECCM (18 parameters)\n")
obj_saturation <- function(phi) {
    if (any(phi < lower_saturation) || any(phi > upper_saturation)) return(1e9)
    res <- evaluate_saturation_eccm_cv(as.numeric(phi), as.integer(dat_all$Resp), as.integer(dat_all$F), as.numeric(dat_all$Bd1), as.numeric(dat_all$Bd2), as.numeric(dat_all$RT), as.numeric(dat_all$ttp), as.numeric(dat_all$ttF), as.integer(dat_all$participant_factor), as.integer(dat_all$is_test), as.integer(20), as.integer(200), as.integer(40), num_participants)
    return(res$test_nlls[1]) 
}
cma_saturation <- cma_es(init_saturation, obj_saturation, lower = lower_saturation, upper = upper_saturation, control = list(maxit = 35, trace = FALSE, sigma = 0.2))
res_saturation <- evaluate_saturation_eccm_cv(as.numeric(cma_saturation$par), as.integer(dat_all$Resp), as.integer(dat_all$F), as.numeric(dat_all$Bd1), as.numeric(dat_all$Bd2), as.numeric(dat_all$RT), as.numeric(dat_all$ttp), as.numeric(dat_all$ttF), as.integer(dat_all$participant_factor), as.integer(dat_all$is_test), as.integer(20), as.integer(200), as.integer(40), num_participants)

saturation_test_nlls <- res_saturation$test_nlls[2:(num_participants+1)]
saturation_metrics <- calc_metrics(res_saturation)

cat("\n======================================\n")
cat("AGGREGATE RESULTS ACROSS 128 PARTICIPANTS\n")
cat("--------------------------------------\n")
cat("SATURATION ECCM\n")
cat(sprintf("Test NLL: %.2f\n", sum(saturation_test_nlls)))
cat(sprintf("PR-AUC  : %.3f\n", saturation_metrics$PRAUC))
cat(sprintf("RT RMSE : %.3f\n", saturation_metrics$RT_RMSE))
cat(sprintf("Accuracy: %.3f\n", saturation_metrics$Accuracy))
cat("--------------------------------------\n")
...
results_df <- data.frame(
    ...
    Saturation_ECCM_NLL = saturation_test_nlls
)
write.csv(results_df, "results/tables/saturation_biological_benchmark.csv", row.names=FALSE)

cat("\n--- PAIRED T-TESTS (Participant Level) ---\n")
t1 <- t.test(pure_eccm_test_nlls, saturation_test_nlls, paired=TRUE)
cat(sprintf("Pure ECCM vs Saturation ECCM: t = %.2f, p = %.3e (Mean Diff: %.2f)\n", t1$statistic, t1$p.value, t1$estimate))

t2 <- t.test(m1_test_nlls, saturation_test_nlls, paired=TRUE)
cat(sprintf("M1 (WSLS) vs Saturation ECCM: t = %.2f, p = %.3e (Mean Diff: %.2f)\n", t2$statistic, t2$p.value, t2$estimate))
