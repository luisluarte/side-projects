...
List evaluate_pcci_eccm_cv(
  ...
  double beta_pfc = phi_18d[17]; // 18th parameter: Prefrontal Cortex (PFC) WSLS weight
  ...
  for (int s = 0; s < num_participants; ++s) {
    ...
    for (int t=0; t<N_t; ++t) {
        ...
        double v_t_ddm = beta_v_eccm * D_t + beta_pfc * wsls_signal;
...
lower_pcci <- c(lower_eccm, 0.0)
upper_pcci <- c(upper_eccm, 5.0)
init_pcci <- c(init_eccm, 1.0)

cat("\nTraining PCCI ECCM (18 parameters)\n")
obj_pcci <- function(phi) {
    if (any(phi < lower_pcci) || any(phi > upper_pcci)) return(1e9)
    res <- evaluate_pcci_eccm_cv(as.numeric(phi), as.integer(dat_all$Resp), as.integer(dat_all$F), as.numeric(dat_all$Bd1), as.numeric(dat_all$Bd2), as.numeric(dat_all$RT), as.numeric(dat_all$ttp), as.numeric(dat_all$ttF), as.integer(dat_all$participant_factor), as.integer(dat_all$is_test), as.integer(20), as.integer(200), as.integer(40), num_participants)
    return(res$test_nlls[1]) 
}
cma_pcci <- cma_es(init_pcci, obj_pcci, lower = lower_pcci, upper = upper_pcci, control = list(maxit = 35, trace = FALSE, sigma = 0.2))
res_pcci <- evaluate_pcci_eccm_cv(as.numeric(cma_pcci$par), as.integer(dat_all$Resp), as.integer(dat_all$F), as.numeric(dat_all$Bd1), as.numeric(dat_all$Bd2), as.numeric(dat_all$RT), as.numeric(dat_all$ttp), as.numeric(dat_all$ttF), as.integer(dat_all$participant_factor), as.integer(dat_all$is_test), as.integer(20), as.integer(200), as.integer(40), num_participants)

pcci_test_nlls <- res_pcci$test_nlls[2:(num_participants+1)]
pcci_metrics <- calc_metrics(res_pcci)
...
cat(sprintf("RT RMSE : %.3f\n", pcci_metrics$RT_RMSE))
cat(sprintf("Accuracy: %.3f\n", pcci_metrics$Accuracy))
cat("--------------------------------------\n")
...
results_df <- data.frame(
    ...
    PCCI_ECCM_NLL = pcci_test_nlls
)
write.csv(results_df, "results/tables/pcci_biological_benchmark.csv", row.names=FALSE)

cat("\n--- PAIRED T-TESTS (Participant Level) ---\n")
t1 <- t.test(pure_eccm_test_nlls, pcci_test_nlls, paired=TRUE)
cat(sprintf("Pure ECCM vs PCCI ECCM: t = %.2f, p = %.3e (Mean Diff: %.2f)\n", t1$statistic, t1$p.value, t1$estimate))

t2 <- t.test(m1_test_nlls, pcci_test_nlls, paired=TRUE)
cat(sprintf("M1 (WSLS) vs PCCI ECCM: t = %.2f, p = %.3e (Mean Diff: %.2f)\n", t2$statistic, t2$p.value, t2$estimate))
