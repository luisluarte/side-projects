  ...
  real partial_sum(array[] int slice_subj,
    ...
    for (idx in 1:size(slice_subj)) {
      ...
      for (t in start_idx[s]:end_idx[s]) {
        ...
        // Smooth Softplus: log(1 + exp(-10 * M_align)) / 10
...
transformed parameters {
  ...
  for (s in 1:N_subj) {
    ...
    tau_decay[s]     = exp(mu_res_raw[4] + sigma[7] * z[7, s]);
    golgi_scale[s]   = exp(mu_res_raw[5] + sigma[8] * z[8, s]);
    w_cb[s]          = exp(mu_res_raw[6] + sigma[9] * z[9, s]);
    w_ctx[s]         = exp(mu_res_raw[7] + sigma[10] * z[10, s]);
    beta_mismatch[s] = exp(mu_res_raw[8] + sigma[11] * z[11, s]);
  }
}
model {
