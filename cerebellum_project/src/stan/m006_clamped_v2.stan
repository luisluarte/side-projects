data {
  ...
  matrix[N_subj, 32] W_exp; // Host-Level Anatomical Matrix (Fixed)
}
parameters {
  vector[9] mu_raw;
...
transformed parameters {
  ...
  // Parameter mapping (Unbounded variables where needed for gradients)
  vector[N_subj] a_base_raw = theta[1, ]'; 
  vector[N_subj] tnd = to_vector(min_rt) .* inv_logit(theta[2, ]');
  vector[N_subj] v_ctx = exp(theta[3, ]');
  vector[N_subj] alpha_ctx = inv_logit(theta[4, ]');
  vector[N_subj] alpha_pc = inv_logit(theta[5, ]');
...
model {
  ...
  // Unbounded latent Purkinje trace
  matrix[N_subj, 32] U_PC = rep_matrix(0.0, N_subj, 32);
  ...
  for (t in 1:N_trials) {
    ...
    for (i in 1:32) {
      // 1. Asymptotic Synaptic Saturation (Smooth bound at +/- 3.0)
      real w_eff = 3.0 * tanh(U_PC[s, i] / 3.0);
      
      // 2. Fractional Memory updates
      ...
      // 3. Continuous Golgi Masking
      real S_mask = tanh(golgi_scale[s] * abs(w_eff * Z[s, i]));
    ...
    // Smooth zero drift prevention (replace hard if with pseudo-Huber or small offset)
    // Using a tiny constant avoids branch-divergence in autodiff
    real safe_veff = (veff >= 0) ? (veff + 1e-4) : (veff - 1e-4);
    
    // 4. Asymptotic Epistemic Boundary (Smooth logistic bound in [0.1, 5.0])
    real raw_a_dyn = a_base_raw[s] + w_u[s] * abs(cb0 * cb1);
    real a_dyn = 0.1 + 4.9 * inv_logit(raw_a_dyn);
    
    // Wiener Likelihood
    target += wiener_lpdf(rt[t] | a_dyn, tnd[s], 0.5, safe_veff);
    ...
    // Latent Unbounded Purkinje Learning Update
    for (i in 1:32) {
      real err_sig = 0.0;
