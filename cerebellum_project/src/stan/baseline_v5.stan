functions {
  real partial_sum(array[] int slice_t_idx,
    ...
    for (i in 1:chunk_size) {
      ...
      // Dynamic boundary purely driven by pre-computed rolling entropy covariate!
      a_dyn_vec[i] = a_s + b_ent_s * entropy[orig_i];
      tnd_vec[i] = tnd_s;
...
data {
  ...
  array[N_trials] real entropy; // Pre-computed EMA rolling entropy
...
transformed parameters {
  ...
  vector[N_subj] beta_ent = beta_max * inv_logit(mu_raw[7] + sigma[7] * z[7]'); // Modulates threshold based on EMA entropy
...
model {
  ...
  target += reduce_sum(partial_sum, t_idx, 1, subj, resp, reward, rt, clean_iti, entropy, a_base, tnd, v_ctx, alpha_win, alpha_loss, k_decay, beta_ent);
}
generated quantities {
  vector[N_trials] log_lik;
  {
    ...
    for (i in 1:N_trials) {
      ...
      real Q_diff = Q[1] - Q[2];
      real a_dyn = a_base[s] + beta_ent[s] * entropy[i];
