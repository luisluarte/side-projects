...
transformed parameters {
  vector[11] theta_unc = theta_mean + L_Sigma * theta_raw;
  ...
  for (s in 1:N_subj) {
    ...
    // SAFE CAPS FOR PATHFINDER (avoid exp() infinity)
    tau_decay[s]     = 100.0 * inv_logit(mu_res_raw[4] + sigma[7] * z[7, s]);
    golgi_scale[s]   = 50.0 * inv_logit(mu_res_raw[5] + sigma[8] * z[8, s]);
    w_cb[s]          = 50.0 * inv_logit(mu_res_raw[6] + sigma[9] * z[9, s]);
    w_ctx[s]         = 50.0 * inv_logit(mu_res_raw[7] + sigma[10] * z[10, s]);
    beta_mismatch[s] = 50.0 * inv_logit(mu_res_raw[8] + sigma[11] * z[11, s]);
  }
}
model {
  ...
  // Implicit priors due to inv_logit bounds: 
  // No explicit log Jacobians needed because we aren't transforming variables in model block
  ...
  target += reduce_sum(partial_sum, seq_subj, grainsize, 
                       ...
                       a_base_raw, tnd, v_ctx, aw, al, alpha_pc, tau_decay, golgi_scale, w_cb, w_ctx, beta_mismatch, 
