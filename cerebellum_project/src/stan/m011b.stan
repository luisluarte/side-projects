functions {
  real partial_sum(array[] int slice_t_idx, int start, int end, array[] int subj, array[] int resp, array[] real reward, array[] real rt, array[] real clean_iti, matrix W_exp, vector a_base, vector tnd, vector v_ctx, vector alpha_win, vector alpha_loss, vector alpha_pc, vector golgi_scale, vector tau_decay, vector beta_conf) {
    real pt = 0.0;
    int current_s = -1;
    vector[2] Q;
    vector[4] frac_mem;
    ...
    real tnd_s; real a_s; real v_s; real aw_s; real al_s; real apc_s; real golgi_s; real inv_tau_s; real b_conf_s;
    ...
    for (i in start:end) {
      ...
      if (s != current_s) {
        ...
        apc_s = alpha_pc[s]; golgi_s = golgi_scale[s];
      ...
      real veff_raw = 18.51 * tanh(v_s * (Q[1] - Q[2]));
...
parameters {
  vector[9] mu_raw; vector<lower=0>[9] sigma; matrix[9, N_subj] z;
}
transformed parameters {
  ...
  vector[N_subj] golgi_scale = exp(mu_raw[7] + sigma[7] * z[7]');
  vector[N_subj] tau_decay = 10.0 * inv_logit(mu_raw[8] + sigma[8] * z[8]');
  vector[N_subj] beta_conf = 10.0 * inv_logit(mu_raw[9] + sigma[9] * z[9]');
  for (s in 1:N_subj) {
...
model {
  ...
  target += reduce_sum(partial_sum, t_idx, 1, subj, resp, reward, rt, clean_iti, W_exp, a_base, tnd, v_ctx, alpha_win, alpha_loss, alpha_pc, golgi_scale, tau_decay, beta_conf);
}
generated quantities {
  vector[N_trials] log_lik;
  {
    ...
    for (i in 1:N_trials) {
      ...
      real veff_raw = 18.51 * tanh((v_ctx[s] * 0.0540248) * (Q[1] - Q[2]));
      real veff = (ch == 1) ? veff_raw : -veff_raw;
