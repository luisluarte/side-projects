...
transformed parameters {
  ...
  vector[N_subj] tnd;
  vector[N_subj] v_ctx = 10.0 * inv_logit(mu_raw[3] + sigma[3] * z[3]');
  vector[N_subj] alpha_win = inv_logit(mu_raw[4] + sigma[4] * z[4]');
  vector[N_subj] alpha_loss = inv_logit(mu_raw[5] + sigma[5] * z[5]');
  vector[N_subj] tau_decay = 10.0 * inv_logit(mu_raw[6] + sigma[6] * z[6]');
  vector[N_subj] beta_conflict = 10.0 * inv_logit(mu_raw[7] + sigma[7] * z[7]');
