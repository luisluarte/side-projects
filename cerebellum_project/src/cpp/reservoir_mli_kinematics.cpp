...
// ==============================================================================
// STATIC MLI FEEDFORWARD INHIBITION RESERVOIR ENGINE
// Granular layer (1,000-D) -> Static MLI layer (200-D, basket & stellate cells)
// Feature Output: X_t = [z_GC, -h_MLI] (1,200 dimensions)
// ==============================================================================
...
List run_mli_reservoir_cpp(
  ...
  // STATIC MLI PROJECTION MATRIX: W_MLI in R^{N_MLI x N_GC} (15% sparse connectivity)
  int mli_fan_in = std::max(5, (int)(0.15 * N_GC));
  std::vector<std::vector<int>> mli_gc_indices(N_MLI, std::vector<int>(mli_fan_in));
  std::vector<std::vector<double>> W_MLI(N_MLI, std::vector<double>(mli_fan_in));
  for (int m = 0; m < N_MLI; ++m) {
    for (int f = 0; f < mli_fan_in; ++f) {
      mli_gc_indices[m][f] = (m * 7 + f * 13) % N_GC;
      // Fixed static random weight
      W_MLI[m][f] = 0.25 + 0.10 * std::sin((double)(m * 3 + f * 7));
    }
  }
  ...
  // Total feature dimensions: N_GC + N_MLI = 1200
  int total_feat_dim = N_GC + N_MLI;
  NumericMatrix X_matrix(N_t, total_feat_dim);
  ...
  for (int t = 0; t < N_t; ++t) {
    ...
    for (int i = 0; i < N_GC; ++i) {
      ...
      X_matrix(t, i) = z_GC_curr[i]; // Direct excitatory channel
    }

    // STATIC MLI FEEDFORWARD INHIBITION: h_MLI = ReLU(W_MLI * z_GC - theta_MLI)
    for (int m = 0; m < N_MLI; ++m) {
      double mli_input = 0.0;
      for (int f = 0; f < mli_fan_in; ++f) {
        mli_input += W_MLI[m][f] * z_GC_curr[mli_gc_indices[m][f]];
      }
      h_MLI[m] = std::max(0.0, mli_input - mli_threshold);
      X_matrix(t, N_GC + m) = -h_MLI[m]; // Feedforward inhibitory channel
    }
