...
// ==============================================================================
// TRIPARTITE KINEMATIC UPGRADE RESERVOIR ENGINE
// 5-Dimensional Mossy Fiber Input with Continuous Temporal Phase Clock:
// u_t = [Resp_prev, Out_prev, Magnitude_curr, Magnitude_diff, exp(-0.1 * delta_t)]^T
// ==============================================================================
...
// [[Rcpp::export]]
List run_tripartite_reservoir_cpp(
  ...
  // 5-dimensional input fan-in (20% of Granule Cells receive temporal clock)
  int mossy_fan_in = 5;
  ...
  for (int t = 0; t < N_t; ++t) {
    ...
    // Continuous temporal clock hazard signal
    double temporal_phase = std::exp(-0.10 * delta_t);

    // 5-dimensional input vector
    double u_arr[5] = {
      (prev_ch == 1) ? 1.0 : -1.0,
      (double)prev_out,
      d_curr,
    ...
    // Granule cell forward pass with temporal clock integration
    ...
    // Autoregressive baseline RT: RT_base = tau_kin * RT_filter + (1 - tau_kin) * mean_rt_global
