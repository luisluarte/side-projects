...
// ==============================================================================
// DEFINITIVE 4-WAY STOCHASTIC TOURNAMENT ENGINE
...
// Model 3: Kernelized Symplectic-HDDM (1,000-D Reservoir + MLI + DCN Rebound Brake)
// ==============================================================================
...
// [[Rcpp::export]]
List precompute_definitive_tournament_subject_cpp(
  ...
  for (int t = 0; t < N_t; ++t) {
    ...
    // --- MODEL 3: KERNELIZED SYMPLECTIC-HDDM ---
    double u_arr[4] = {
      (prev_ch == 1) ? 1.0 : -1.0,
      (double)prev_out,
      d_curr,
    ...
    // DCN Rebound non-linearity g(S_t) = 2.85 * S_t^1.75
    double dcn_rebound = 2.85 * std::pow(norm_entropy, 1.75);
    rebound_m3_vec[t] = dcn_rebound;
    ...
    // Correct directional Action-Selection Logit incorporating WSLS base + magnitude + microcircuit policy
    double s_ch = (prev_ch == 1) ? 1.0 : -1.0;
    double logit_bias = 0.0;
    if (prev_out == 1) {
      logit_bias = s_ch * (1.0 + w_mag_curr * d_curr); // Stay with magnitude
    } else {
      logit_bias = -s_ch * (1.0 + w_mag_alt * d_diff);  // Switch with magnitude
    }
...
// [[Rcpp::export]]
double compute_tournament_deviance_cpp(
    const IntegerVector& resp_R,
    const NumericVector& rt_R,
...
// [[Rcpp::export]]
List evaluate_definitive_model_cpp(
    const IntegerVector& resp_R,
    const NumericVector& rt_R,
    const NumericVector& v_raw_R,
    const NumericVector& mod_raw_R,
  ...
  for (int t = 0; t < N_t; ++t) {
    ...
    // Discrete choice accuracy: does predicted direction match actual choice?
