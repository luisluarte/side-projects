...
cat("Initiating Global Hierarchical CMA-ES Audit...\n")
...
# Generate W_exp off-compiler
set.seed(42)
W_exp <- matrix(rnorm(N_subj * 32, 0, 1), nrow=N_subj, ncol=32)

sourceCpp("src/models/hierarchical_map_m006.cpp")

n_params <- 9 + 9 + 9 * N_subj
cat(sprintf("Optimizing %d dimensions...\n", n_params))

fit_fn <- function(x) {
    g_mu <- x[1:9]
    g_sig <- x[10:18]
    z <- matrix(x[19:n_params], nrow=9, ncol=N_subj)
    val <- get_hierarchical_map_006(g_mu, g_sig, z, dat_clean$participant_idx, dat_clean$Boundary, 
...
cma_es_custom <- function (par, fn, lower, upper) {
    ...
    return(list(C=C, par=xmean, vals=e$values, vecs=e$vectors, fail="maxiter"))
}

lower_bounds <- c(rep(-5, 9), rep(0, 9), rep(-5, 9*N_subj))
upper_bounds <- c(rep(5, 9), rep(5, 9), rep(5, 9*N_subj))
init_par <- c(rep(0, 9), rep(0.5, 9), rep(0, 9*N_subj))

res <- cma_es_custom(init_par, fit_fn, lower_bounds, upper_bounds)

# Phase III: Spectral Eigendecomposition
C <- res$C
# Isolate the 18 group parameters (biological core)
C_group <- C[1:18, 1:18]
sigma_vec <- sqrt(diag(C_group))
Corr_group <- C_group / (sigma_vec %o% sigma_vec)

eig_res <- eigen(Corr_group, symmetric=TRUE)

degenerate_idx <- which(eig_res$values < 1e-3)

report <- "### Spectral Diagnostic Report\n\n"
report <- paste0(report, sprintf("Status: %s\n", res$fail))
report <- paste0(report, "Eigenvalues of the Correlation Matrix (Biological Group Parameters):\n")
for(i in 1:18) report <- paste0(report, sprintf("Lambda %d: %.2e\n", i, eig_res$values[i]))

if (length(degenerate_idx) > 0) {
    report <- paste0(report, "\nDegenerate Eigenvectors Detected (< 1e-3):\n")
    param_names <- c("mu_a", "mu_tnd", "mu_v", "mu_alpha_ctx", "mu_alpha_pc", "mu_gamma", "mu_golgi", "mu_tau", "mu_wu",
                     "sig_a", "sig_tnd", "sig_v", "sig_alpha_ctx", "sig_alpha_pc", "sig_gamma", "sig_golgi", "sig_tau", "sig_wu")
    ...
    report <- paste0(report, "\nNo degenerate eigenvalues < 1e-3 detected in the biological subspace.\n")
}

writeLines(report, "results/spectral_diagnostic_report.md")
