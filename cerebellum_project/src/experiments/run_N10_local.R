...
cat("1. Compiling fixed Stan models...\n")
mod_base <- cmdstan_model("src/stan/baseline.stan", cpp_options = list(stan_threads = TRUE))
mod_m006 <- cmdstan_model("src/stan/m006_strict_hmc.stan", cpp_options = list(stan_threads = TRUE))

dat_raw <- read_csv("data/raw/behavioral_compilate.csv", show_col_types=FALSE)
epistemic <- readRDS("results/epistemic_geometry.rds")

test_subjs_10 <- head(epistemic$test_subjs, 10)
...
cat("4. Forward Simulation Metrics...\n")
...
for(s in 1:N_subj) {
    ...
    res_list[[length(res_list)+1]] <- data.frame(subj=s, model="Baseline", roc=b_roc, pr=b_pr)
    res_list[[length(res_list)+1]] <- data.frame(subj=s, model="M006", roc=m_roc, pr=m_pr)
}

df_res <- bind_rows(res_list) %>% filter(!is.na(roc))

cat("\nSummary_Table_N10_Fixed\n")
print(df_res %>% group_by(model) %>% summarise(mean_roc=mean(roc), mean_pr=mean(pr)))
