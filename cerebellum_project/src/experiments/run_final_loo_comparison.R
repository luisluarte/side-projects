...
cat("1. Loading Data and Preparing Stan Environment...\n")
...
cat("2. Processing Baseline RL-DDM...\n")
fit_base <- readRDS("results/hmc_baseline_fit.rds")
loo_base <- fit_base$loo(cores = 4)

cat("3. Processing Optimized MAGI (M006) Log-Likelihoods...\n")
fit_m006 <- readRDS("results/hmc_phase3_fit.rds")
mod_gq <- cmdstan_model("src/stan/m006_gq.stan")

cat("Running Generated Quantities block via CmdStanR...\n")
fit_m006_gq <- mod_gq$generate_quantities(
    fitted_params = fit_m006,
    data = stan_data,
    parallel_chains = 4
)
loo_m006 <- fit_m006_gq$loo(cores = 4)
