...
repo_root = "." if os.path.exists("results/cohort_comparison_metrics_unclamped.csv") else "../.."
results_dir = os.path.join(repo_root, "results")
figures_dir = os.path.join(repo_root, "figures")
os.makedirs(figures_dir, exist_ok=True)

df_comp = pd.read_csv(os.path.join(results_dir, "cohort_comparison_metrics_unclamped.csv"))
df_params = pd.read_csv(os.path.join(results_dir, "m006_unclamped_parameter_distributions.csv"))
...
params_info = [
    ...
    ('gamma', r'Cerebellar Gain $\gamma$', '#c0392b', (0, 10)),
...
plt.suptitle(r'M006 (Pre-Correction / Unclamped) Parameter Distributions Across Cohort ($N = 128$)', fontsize=16, fontweight='bold', y=1.02)
plt.tight_layout()
plt.savefig(os.path.join(figures_dir, "m006_unclamped_parameter_distributions.png"), dpi=200, bbox_inches='tight')
plt.close()

# 2. STATISTICAL COMPARISON (M006 Unclamped vs Base)
...
sns.kdeplot(df_comp['NLL_M006_Unclamped'], ax=axes[0], color='#e74c3c', fill=True, alpha=0.3, label=f"M006 Unclamped (Mean: {df_comp['NLL_M006_Unclamped'].mean():.1f})")
...
axes[1].scatter(delta_sorted, range(1, len(delta_sorted)+1), color='#e74c3c', alpha=0.8, s=20)
axes[1].hlines(range(1, len(delta_sorted)+1), xmin=0, xmax=delta_sorted, color='#e74c3c', alpha=0.3, linewidth=0.8)
axes[1].set_title(r'Subject-Level Delta ($\Delta \mathcal{L} = \mathcal{L}_{Base} - \mathcal{L}_{M006}$)', fontsize=13, fontweight='bold')
...
brier_df = pd.DataFrame({
    'Model': ['Baseline']*len(df_comp) + ['M006 Unclamped']*len(df_comp),
    'Brier': list(df_comp['Brier_Base']) + list(df_comp['Brier_M006'])
})
sns.boxplot(x='Model', y='Brier', data=brier_df, ax=axes[2], palette={'Baseline': '#7f8c8d', 'M006 Unclamped': '#e74c3c'}, boxprops=dict(alpha=0.4), showfliers=False)
sns.stripplot(x='Model', y='Brier', data=brier_df, ax=axes[2], palette={'Baseline': '#7f8c8d', 'M006 Unclamped': '#e74c3c'}, alpha=0.7, jitter=True, size=5)
...
plt.suptitle(r'Full Cohort Benchmark: $M_{006}$ (Pre-Correction / Unclamped) vs. $M_{base}$ ($N=128$)', fontsize=16, fontweight='bold', y=1.05)
plt.tight_layout()
plt.savefig(os.path.join(figures_dir, "m006_unclamped_vs_base_cohort_benchmark.png"), dpi=200, bbox_inches='tight')
plt.close()

print("Unclamped figures successfully generated in figures/")
