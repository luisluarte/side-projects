...
# Model 18: 11 params (adds mf_threshold [unconstrained], explore_gain [log])
# mf_threshold is unconstrained (can be negative = always on, positive = selective)
init_phi_18 <- c(log(2.0), log(0.3/0.7), log(3.0), log(0.5/0.5), log(0.5/0.5), 0.5, log(1.0), log(0.5), log(0.1), 0.0, log(1.0))

cl <- makeCluster(detectCores() - 1)
registerDoParallel(cl)

cat("Running parallel MCMC fits for Model 18: MF-Gated Choice Reversal...\n")
results_list <- foreach(p = participants, .packages=c("Rcpp")) %dopar% {
  ...
  pred_stay_18 <- numeric(nrow(p_data))
  
  for (i in 2:nrow(p_data)) {
    is_stay <- (p_data$Resp[i] == p_data$Resp[i-1])
    actual_stay[i] <- ifelse(is_stay, 1, 0)
    
    pred_ch_18 <- ifelse(metrics_18$prob_ch1[i] > 0.5, 1, 2)
    pred_stay_18[i] <- ifelse(pred_ch_18 == p_data$Resp[i-1], 1, 0)
    
    if (!is_stay) {
      actual_switches <- actual_switches + 1
      if (ifelse(metrics_12$prob_ch1[i] > 0.5, 1, 2) != p_data$Resp[i-1]) pred_switch_12 <- pred_switch_12 + 1
      if (ifelse(metrics_18$prob_ch1[i] > 0.5, 1, 2) != p_data$Resp[i-1]) pred_switch_18 <- pred_switch_18 + 1
      
      conf_switch_12 <- c(conf_switch_12, ifelse(p_data$Resp[i]==1, metrics_12$prob_ch1[i], 1.0 - metrics_12$prob_ch1[i]))
      conf_switch_18 <- c(conf_switch_18, ifelse(p_data$Resp[i]==1, metrics_18$prob_ch1[i], 1.0 - metrics_18$prob_ch1[i]))
    }
  }
  
  TP <- sum(actual_stay[-1] == 1 & pred_stay_18[-1] == 1) / max(1, sum(actual_stay[-1] == 1))
  FN <- sum(actual_stay[-1] == 1 & pred_stay_18[-1] == 0) / max(1, sum(actual_stay[-1] == 1))
  FP <- sum(actual_stay[-1] == 0 & pred_stay_18[-1] == 1) / max(1, sum(actual_stay[-1] == 0))
  TN <- sum(actual_stay[-1] == 0 & pred_stay_18[-1] == 0) / max(1, sum(actual_stay[-1] == 0))
  
  data.frame(
    subject = p,
    switches = actual_switches,
    pred_switch_12 = pred_switch_12,
    pred_switch_18 = pred_switch_18,
    dev_12 = dev_12, dev_18 = dev_18,
    TP = TP, FN = FN, FP = FP, TN = TN,
    conf_12 = mean(conf_switch_12, na.rm=TRUE),
    conf_18 = mean(conf_switch_18, na.rm=TRUE),
    theta_mf = chain_18[iters, 10],
    theta_explore = exp(chain_18[iters, 11])
  )
}
...
recall_18 <- sum(results$pred_switch_18) / sum_switches * 100
...
report <- c(
  ...
  sprintf("*   **Model 18 (MF-Gated Choice Reversal):** %.2f%%", recall_18),
  sprintf("*   **Delta:** %+.2f%%", recall_18 - recall_12),
  "",
  "## Deviance",
  sprintf("*   **Model 12 Mean Deviance:** %.2f", mean(results$dev_12, na.rm=TRUE)),
  sprintf("*   **Model 18 Mean Deviance:** %.2f", mean(results$dev_18, na.rm=TRUE)),
  sprintf("*   **Paired t-test p-value:** %.4e", t.test(results$dev_12, results$dev_18, paired=TRUE)$p.value),
  "",
  "## Switch Confidence",
  sprintf("*   **Model 12:** %.2f%%", mean(results$conf_12, na.rm=TRUE)*100),
  sprintf("*   **Model 18:** %.2f%%", mean(results$conf_18, na.rm=TRUE)*100),
  "",
  ...
  sprintf("*   **MF Threshold (unconstrained):** %.4f", median(results$theta_mf, na.rm=TRUE)),
  sprintf("*   **Explore Gain:** %.4f", median(results$theta_explore, na.rm=TRUE))
)

writeLines(report, "docs/Golgi_Reversal_Results.md")
cat("\nReport successfully generated in docs/Golgi_Reversal_Results.md\n")
