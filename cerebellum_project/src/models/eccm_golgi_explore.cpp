...
// Model 17: Golgi + MF-Gated Exploration
// Combines the proven Divisive Normalization (Model 12) with a threshold-gated
// exploration mechanism. When instantaneous MF energy exceeds a learned threshold,
// the model injects a COUNTER-DRIFT proportional to how far above threshold we are.
// This is biologically analogous to the LC-NE system: a phasic burst (triggered by
// unexpected uncertainty) that directly modulates the evidence accumulation process.
//
// Key difference from Model 16: we don't use GC entropy (which is always high).
// Instead we use the RAW MF energy spike — the actual predictor of switches.
// And instead of multiplying the drift (which can catastrophically flip sign on
// every trial), we ADD a counter-drift term that only activates above threshold.

// [[Rcpp::export]]
inline double eval_eccm_golgi_explore(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& delta_t) {
    ...
    double theta_golgi = std::exp(phi[8]);   // Golgi divisive gain (from Model 12)
    double mf_threshold = std::exp(phi[9]);  // MF energy threshold for exploration trigger
    double explore_gain = std::exp(phi[10]); // Strength of the counter-drift when above threshold
    ...
    double prev_mf_energy = 0.0; // Track previous trial's MF energy for delta
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        // Golgi Divisive Normalization (proven from Model 12)
        ...
        // EXPLORATION GATE: Delta MF energy triggers counter-drift
        double delta_mf = l2_mf - prev_mf_energy;
        if (delta_mf > mf_threshold) {
            // Inject counter-drift: push AGAINST the current drift direction
            double counter = explore_gain * (delta_mf - mf_threshold);
            v_effective -= std::copysign(counter, v_effective);
        }
