...
inline double calc_expected_rt_dist_ar(double v, double a, double w, double t_nd) {
    double z = w * a;
    if (std::abs(v) < 1e-4) {
        return t_nd + z * (a - z);
    } else {
...
// [[Rcpp::export]]
NumericVector extract_ll_bvk_full_gating_ar_fatigue(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur, const NumericVector& ttp, int N_MF) {
    ...
    double beta_scale = std::exp(phi[11]);
    double beta_shape = std::exp(phi[12]);
    ...
    for (int t = 0; t < T; ++t) {
        ...
        double delta_rt_prev = 0.0;
        if (t > 1) {
            delta_rt_prev = rt[t-1] - rt[t-2];
        }
        
        double a_base_t = a_0 + rho_ar * delta_rt_prev;
        if (a_base_t < 0.01) a_base_t = 0.01;
        
        double t_elapsed = ttp[t] / 1000.0;
        double a_effective = (a_base_t + beta_scale * std::pow(conflict, beta_shape)) * std::exp(-u * t_elapsed);
...
// [[Rcpp::export]]
NumericVector extract_rt_bvk_full_gating_ar_fatigue(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur, const NumericVector& ttp, int N_MF) {
    double alpha_ctx = 1.0 / (1.0 + std::exp(-phi[0]));
    double eta_gc = 0.5 + std::exp(phi[1]);
    double lambda_gc = std::exp(phi[2]);
    double theta_cb = std::exp(phi[3]);
    ...
    for (int t = 0; t < T; ++t) {
        ...
        pred_rt[t] = calc_expected_rt_dist_ar(safe_v, a_effective, w_bias, tau_nd);
