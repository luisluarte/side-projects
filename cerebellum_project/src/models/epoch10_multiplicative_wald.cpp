...
// [[Rcpp::export]]
NumericVector extract_epoch10_multiplicative(const std::vector<double>& phi, const std::vector<double>& hyper, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt) {
    int N = 100; 
    ...
    double gamma_perturb = 1.0 / (1.0 + std::exp(-phi[6])); // Bounded (0,1) for gain scale
    double scale_I = std::exp(phi[7]); 
    double gamma_spectral = std::exp(phi[8]); 
    ...
    for (int t = 0; t < T; ++t) {
        ...
        // IV. Multiplicative Effective Value Hybridization (DCN Gain)
        ...
        // VI. Plasticity Updates (LASSO Subgradient)
        double RPE = R - Q_ctx[ch];
        Q_ctx[ch] += alpha_ctx * RPE;
        
        for(int i=0; i<N; ++i) {
            W_cb[i] += alpha_cb * RPE * h[i] - lambda_lasso * signum(W_cb[i]);
        }
        RPE_ctx_last = RPE;
