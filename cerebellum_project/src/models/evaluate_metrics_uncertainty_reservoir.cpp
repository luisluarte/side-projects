...
// [[Rcpp::export]]
List eval_metrics_eccm_uncertainty_reservoir(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& delta_t) {
    ...
    double tau = 1.0 / (1.0 + std::exp(-phi[7]));
    double alpha_out = 1.0 / (1.0 + std::exp(-phi[8]));
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        double epsilon = 1.0 / (1.0 + std::exp(-U_t));
        eps_trace.push_back(epsilon);
    ...
    return List::create(Named("prob_ch1") = prob_ch1, Named("exp_rt") = exp_rt, Named("epsilon") = eps_trace);
}
