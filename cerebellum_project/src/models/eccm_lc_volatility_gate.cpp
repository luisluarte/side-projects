...
inline double calc_expected_rt_dist_lc(double v, double a, double w, double t_nd) {
    double z = w * a;
    if (std::abs(v) < 1e-4) {
        return t_nd + z * (a - z);
    } else {
...
// [[Rcpp::export]]
NumericVector extract_ll_lc_volatility_gate(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur, const NumericVector& ttp, int N_MF) {
    ...
    double alpha_vol = 1.0 / (1.0 + std::exp(-phi[11]));
    double beta_vol_a = std::exp(phi[12]);
    ...
    for (int t = 0; t < T; ++t) {
        ...
        double a_effective = (a_base + beta_a * conflict + beta_vol_a * Omega_t) * std::exp(-u * t_elapsed);
        ...
        // Locus Coeruleus Volatility Gate Update
...
// [[Rcpp::export]]
inline double eval_lc_volatility_gate(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur, const NumericVector& ttp, int N_MF) {
    NumericVector ll = extract_ll_lc_volatility_gate(phi, resp, out, rt, iti, f_dur, ttp, N_MF);
    double nll = 0.0;
...
// [[Rcpp::export]]
NumericVector extract_rt_lc_volatility_gate(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur, const NumericVector& ttp, int N_MF) {
    double alpha_ctx = 1.0 / (1.0 + std::exp(-phi[0]));
    double eta_gc = 0.5 + std::exp(phi[1]);
    double lambda_gc = std::exp(phi[2]);
    double theta_cb = std::exp(phi[3]);
    ...
    for (int t = 0; t < T; ++t) {
        ...
        pred_rt[t] = calc_expected_rt_dist_lc(safe_v, a_effective, w_bias, tau_nd);
