...
// [[Rcpp::export]]
inline double eval_eccm_switch_mechanisms(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& delta_t) {
    ...
    double gamma_suppress = std::exp(phi[7]); // Disagreement sensitivity
    
    int k = 80; int N_MF = 240, N_GC = 1024, N_MLI = 256; // Expanded Mossy Fibers
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        // Suppress drift rate using disagreement
        ...
        for (int s = 0; s < shifts; ++s) {
            // Shift block 1 (Choices), block 2 (Rewards), block 3 (TD Errors)
            for(int j=k-2; j>=0; --j) { 
                mf[j+1] = mf[j]; 
                mf[k+j+1] = mf[k+j]; 
                mf[2*k+j+1] = mf[2*k+j];
            ...
            if (s == 0) {
                mf[0] = (ch == 1) ? 1.0 : -1.0; 
                mf[k] = R_raw;
                mf[2*k] = err_cb; // Store prediction error
            } else {
                mf[0] = 0.0; 
                mf[k] = 0.0;
                mf[2*k] = 0.0;
