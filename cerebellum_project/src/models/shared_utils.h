...
// Wiener PDF with dynamic starting point w (relative, 0 to 1)
inline double wiener_pdf_w(double t_raw, int choice, double v, double a, double t_nd, double w_param, double eps = 1e-9) {
  double t = t_raw - t_nd;
  if (t <= 0.001) return 1e-12;
  double sign = (choice == 1) ? 1.0 : -1.0;
  // x0 is the relative starting position for the absorbing boundary of the chosen response
  double x0 = (choice == 1) ? (1.0 - w_param) : w_param;
  double drift_term = std::exp(sign * v * a * w_param - 0.5 * v * v * t);
  double tt = t / (a * a);
  double sum = 0.0;
  if (tt >= 0.08) {
    for (int k = 1; k <= 30; ++k) {
...
inline double calc_pen_ll(const std::vector<double>& D_vec) {
    int N = D_vec.size();
    if (N == 0) return 1e9;
    // we do this to compute the gradient for our custom LL
