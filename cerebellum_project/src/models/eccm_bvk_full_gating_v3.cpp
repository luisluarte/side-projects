...
inline double calc_expected_rt_dist_v3(double v, double a, double w, double t_nd) {
    double z = w * a;
    if (std::abs(v) < 1e-4) {
        return t_nd + z * (a - z);
    } else {
...
// [[Rcpp::export]]
inline double eval_bvk_full_gating_v3(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur, const NumericVector& ttp, int N_MF) {
    ...
    double kappa_cb = std::exp(phi[5]);
    double a_base = std::exp(phi[6]);
    double beta_a = std::exp(phi[7]);
    double u = phi[8] * 0.001; 
    double tau_min = 0.5 / (1.0 + std::exp(-phi[9]));
    double delta_tau_base = std::exp(phi[10]);
    double kappa_auto = std::exp(phi[11]);
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        double v_effective = kappa_ctx * delta_Q_ctx + kappa_cb * delta_Q_cb;
        double safe_v = std::abs(v_effective) < 1e-4 ? (v_effective >= 0 ? 1e-4 : -1e-4) : v_effective;
        
        double t_elapsed = ttp[t] / 1000.0;
        double a_effective = (a_base + beta_a * conflict) * std::exp(-u * t_elapsed);
        if (a_effective < 0.01) a_effective = 0.01;
        if (a_effective > 10.0) a_effective = 10.0;
        
        double t_nd_t = tau_min + delta_tau_base * std::exp(-kappa_auto * std::abs(delta_Q_cb));
...
// [[Rcpp::export]]
NumericVector extract_rt_bvk_full_gating_v3(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur, const NumericVector& ttp, int N_MF) {
    double alpha_ctx = 1.0 / (1.0 + std::exp(-phi[0]));
    double eta_gc = 0.5 + std::exp(phi[1]);
    double lambda_gc = std::exp(phi[2]);
    double theta_cb = std::exp(phi[3]);
    ...
    for (int t = 0; t < T; ++t) {
        ...
        pred_rt[t] = calc_expected_rt_dist_v3(safe_v, a_effective, w_bias, t_nd_t);
...
// [[Rcpp::export]]
NumericVector extract_ll_bvk_full_gating_v3(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur, const NumericVector& ttp, int N_MF) {
    double alpha_ctx = 1.0 / (1.0 + std::exp(-phi[0]));
    double eta_gc = 0.5 + std::exp(phi[1]);
    double lambda_gc = std::exp(phi[2]);
    double theta_cb = std::exp(phi[3]);
    ...
    for (int t = 0; t < T; ++t) {
        ...
        int w_choice = (resp[t] == 2) ? 1 : 2;
        ll[t] = std::log(wiener_pdf_w(rt[t], w_choice, safe_v, a_effective, t_nd_t, w_bias));
