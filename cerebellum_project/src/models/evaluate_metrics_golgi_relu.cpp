...
// [[Rcpp::export]]
List eval_metrics_eccm_golgi_relu(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& delta_t) {
    ...
    double theta_sub = std::exp(phi[8]);
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        for (int i=0; i<N_GC; ++i) {
            ...
            gc[i] = std::max(0.0, std::min(1.0, act - threshold));
        }
