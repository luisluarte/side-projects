...
// [[Rcpp::export]]
NumericVector extract_epoch10_2_hybrid(const std::vector<double>& phi, const std::vector<double>& hyper, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt) {
    int N = 100; 
    ...
    double gamma_perturb = 1.0 / (1.0 + std::exp(-phi[6])); 
    double scale_I = std::exp(phi[7]); 
    double gamma_spectral = std::exp(phi[8]); 
    
    double sigma_diff = std::exp(phi[9]);
    double sigma_tnd  = std::exp(phi[10]);
    double delta_cb   = phi[11];
