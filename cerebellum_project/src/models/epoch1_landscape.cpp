...
// [[Rcpp::export]]
NumericVector extract_topo_7(const std::vector<double>& phi, const std::vector<int>& genes, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur, bool return_ll) {
    // Topo 7: Striatal-Cerebellar Partition
    ...
    double theta_cb = phi[10]; 
    double beta_cb_v = phi[11];
    ...
    for (int t = 0; t < T; ++t) {
        ...
        // Strict Partition: Cerebellum acts as high-frequency drift norm/gain, Striatum acts on macroscopic variance
        double v_effective = kappa_v * (delta_Q_ctx);
        if(genes[0] == 1) v_effective *= std::exp(beta_cb_v * delta_Q_cb); // Cerebellar Gain
        if(genes[1] == 1) v_effective *= std::exp(beta_str_v * (0.5 - Str_bar)); // Striatal Gain
        ...
        if(genes[2] == 1) a_effective += beta_str_a * (0.5 - Str_bar);
...
// [[Rcpp::export]]
inline double eval_topo_7(const std::vector<double>& phi, const std::vector<int>& genes, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur) {
    NumericVector ll = extract_topo_7(phi, genes, resp, out, rt, iti, f_dur, true);
    double nll = 0.0;
    for(int i=0; i<ll.size(); ++i) nll -= 2.0 * ll[i];
    return (std::isnan(nll) || std::isinf(nll)) ? 1e9 : nll;
}
