...
// [[Rcpp::export]]
inline double eval_eccm_cortical_rpe(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& delta_t) {
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        double err_cb = 0.0;
        double err_ctx = 0.0; // Cortical Reward Prediction Error
        ...
        for (int s = 0; s < shifts; ++s) {
            ...
            if (s == 0) {
                mf[0] = (ch == 1) ? 1.0 : -1.0; 
                mf[k] = R_raw;
                mf[2*k] = err_ctx; // Store CORTICAL prediction error instead of cerebellar error
            } else {
                mf[0] = 0.0; 
                mf[k] = 0.0;
                mf[2*k] = 0.0;
