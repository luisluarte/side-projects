...
// [[Rcpp::export]]
inline double eval_eccm_smooth_graph(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& delta_t) {
    ...
    double alpha_diff = std::exp(phi[8]); // Spatial diffusion rate
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        // Continuous temporal smoothing mapped via bandwidth alpha_diff
