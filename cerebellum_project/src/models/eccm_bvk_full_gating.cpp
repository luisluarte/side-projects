...
// Analytical LTI Propagator for Mossy Fiber Cascade
inline std::vector<double> exact_mf_step(double dt, const std::vector<double>& mf, double tau_m, double I_drive, int N_MF) {
...
// [[Rcpp::export]]
inline double eval_bvk_full_gating(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur) {
    ...
    for (int t = 0; t < resp.size(); ++t) {
        ...
        if (iti[t] > 0.01) {
            mf_state = exact_mf_step(iti[t], mf_state, tau_m, 0.0, N_MF);
            double decay_gc_iti = std::exp(-lambda_gc * iti[t]);
            double decay_mli_iti = std::exp(-(lambda_gc * 1.5) * iti[t]);
            
            for (int k=0; k<N_MF; ++k) {
                w_gc1[k] *= decay_gc_iti;
                w_gc2[k] *= decay_gc_iti;
                w_mli1[k] *= decay_mli_iti;
                w_mli2[k] *= decay_mli_iti;
            }
        }
        ...
        // Q[1] is Upper, Q[0] is Lower
        double delta_Q_ctx = Q_ctx[1] - Q_ctx[0];
        double delta_Q_cb = Q_cb_2 - Q_cb_1;
        
        double w_bias = 0.5 + 0.45 * std::tanh(theta_cb * delta_Q_cb); // Absolute bias
        double conflict = 0.5 * (1.0 - std::tanh(10.0 * delta_Q_ctx) * std::tanh(10.0 * delta_Q_cb));
        
        double v_base = kappa_ctx * delta_Q_ctx + kappa_cb * delta_Q_cb; // Absolute drift
        ...
        if (f_dur[t] > 0.01) {
            mf_state = exact_mf_step(f_dur[t], mf_state, tau_m, reward, N_MF);
            ...
            double scale_gc1 = eta_gc * E_cb1 * int_gc_f;
            double scale_gc2 = eta_gc * E_cb2 * int_gc_f;
            double scale_mli1 = -eta_mli * E_cb1 * int_mli_f;
            double scale_mli2 = -eta_mli * E_cb2 * int_mli_f;
...
// [[Rcpp::export]]
NumericVector extract_ll_bvk_full_gating(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur) {
    double alpha_ctx = 1.0 / (1.0 + std::exp(-phi[0]));
    double tau_m = 1.0 + std::exp(phi[1]);
    double eta_gc = 0.5 + std::exp(phi[2]);
    double lambda_gc = std::exp(phi[3]);
...
// [[Rcpp::export]]
inline double eval_bvk_full_gating_poly(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur) {
    double alpha_ctx = 1.0 / (1.0 + std::exp(-phi[0]));
    double tau_m = 1.0 + std::exp(phi[1]);
    double eta_gc = 0.5 + std::exp(phi[2]);
    double lambda_gc = std::exp(phi[3]);
...
// [[Rcpp::export]]
NumericVector extract_ll_bvk_full_gating_poly(const std::vector<double>& phi, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur) {
    double alpha_ctx = 1.0 / (1.0 + std::exp(-phi[0]));
    double tau_m = 1.0 + std::exp(phi[1]);
    double eta_gc = 0.5 + std::exp(phi[2]);
    double lambda_gc = std::exp(phi[3]);
