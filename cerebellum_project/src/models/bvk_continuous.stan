functions {
  ...
  real partial_sum(
    ...
    data int N_MF,
    vector alpha_ctx, vector tau_m, vector eta_gc, vector lambda_gc, vector eta_mli, vector lambda_mli,
    vector theta_cb, vector kappa_ctx, vector gamma_suppress, vector a, vector tau_nd
  ) {
    real target_sum = 0.0;
...
model {
  ...
  mu_eta_mli ~ normal(0, 1.5);
  mu_lambda_mli ~ normal(0, 1.5);
  ...
  sigma_eta_mli ~ normal(0, 1);
  sigma_lambda_mli ~ normal(0, 1);
  ...
  z_eta_mli ~ std_normal();
  z_lambda_mli ~ std_normal();
  ...
  target += reduce_sum(
    ...
    choice, rt, reward, iti, f_dur, start_idx, end_idx,
    N_MF,
    alpha_ctx, tau_m, eta_gc, lambda_gc, eta_mli, lambda_mli, theta_cb, kappa_ctx, gamma_suppress, a, tau_nd
  );
}