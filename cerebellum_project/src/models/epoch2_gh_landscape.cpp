...
// GAUSS-HERMITE QUADRATURE for O(1) Convolution
inline double convolved_density_gh(double RT, int choice, double v, double a, double mu_tnd, double sigma_tnd) {
    if (sigma_tnd < 1e-4) {
        if (RT > mu_tnd) {
    ...
    double x[5] = {0.0, 0.9585725, -0.9585725, 2.0201829, -2.0201829};
    double w[5] = {0.9453087, 0.3936193, 0.3936193, 0.0199532, 0.0199532};
    
    double sum = 0.0;
    for(int i=0; i<5; ++i) {
        double tau = RT - mu_tnd - std::sqrt(2.0) * sigma_tnd * x[i];
        if (tau > 0) {
            sum += w[i] * wiener_pdf_w(tau, choice, v, a, 0.0, 0.5);
        }
    }
    
    double prob = sum / std::sqrt(M_PI);
    return (prob > 1e-9) ? prob : 1e-9;
}

// [[Rcpp::export]]
NumericVector extract_epoch2_gh(const std::vector<double>& phi, const std::vector<double>& hyper, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, bool return_ll) {
    int N = 100;
    ...
    for (int t = 0; t < T; ++t) {
        ...
        if (return_ll) {
            out_vec[t] = std::log(convolved_density_gh(rt[t], w_choice, safe_v, a_effective, mu_tnd, sigma_tnd));
        } else {
...
// [[Rcpp::export]]
inline double eval_epoch2_gh(const std::vector<double>& phi, const std::vector<double>& hyper, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt) {
    NumericVector ll = extract_epoch2_gh(phi, hyper, resp, out, rt, true);
    double nll = 0.0;
    for(int i=0; i<ll.size(); ++i) nll -= 2.0 * ll[i];
    return (std::isnan(nll) || std::isinf(nll)) ? 1e9 : nll;
}
