...
// [[Rcpp::export]]
NumericVector extract_topology_v4(const std::vector<double>& phi, const std::vector<int>& genes, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur, const NumericVector& ttp, bool return_ll) {
    
    int TOPOLOGY = genes[0]; // 4 = Cerebellar Amplifier
    int G1 = genes[1]; 
    int G2 = genes[2]; 
    int G3 = genes[3]; 
    int G4 = genes[4]; 
    ...
    // Decoupled polynomial fatigue (biological baseline control)
    double b_p1 = phi[16];
    double b_p2 = phi[17];
    double b_p3 = phi[18];
    ...
    for (int t = 0; t < T; ++t) {
        ...
        // TOPOLOGY 4: Cerebellar Amplifier
        // Instead of additive utility, Cerebellum acts as exponential gain on Cortex drift
        double v_effective = kappa_v * delta_Q_ctx;
        if (TOPOLOGY == 4) {
            // sign of delta_Q_ctx times magnitude amplified by cerebellar prediction
            v_effective = v_effective * std::exp(theta_cb * delta_Q_cb); 
        } else {
        ...
        // Biological limits to prevent explosive drift
        if (std::abs(v_effective) > 10.0) {
            v_effective = (v_effective > 0 ? 10.0 : -10.0);
        }
        ...
        double poly_fatigue = b_p1 * norm_t + b_p2 * std::pow(norm_t, 2) + b_p3 * std::pow(norm_t, 3);
        a_effective += poly_fatigue;
        
        if (a_effective < 0.01) a_effective = 0.01;
        if (a_effective > 10.0) a_effective = 10.0;
        
        double w_bias = 0.5; // Neutral bias, evidence strictly drives response
        ...
        if (TOPOLOGY == 4) { 
            // In Amplifier geometry, Cerebellum still learns to predict reward, 
            // but is suppressed by cortex confidence
            RPE_cb = (reward - cb_pred) * (1.0 - std::abs(RPE_ctx));
        }
...
// [[Rcpp::export]]
inline double eval_magi_topo_v4(const std::vector<double>& phi, const std::vector<int>& genes, const IntegerVector& resp, const IntegerVector& out, const NumericVector& rt, const NumericVector& iti, const NumericVector& f_dur, const NumericVector& ttp) {
    NumericVector ll = extract_topology_v4(phi, genes, resp, out, rt, iti, f_dur, ttp, true);
    double nll = 0.0;
