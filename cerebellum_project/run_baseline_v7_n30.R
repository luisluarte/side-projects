...
mod_v7 <- cmdstan_model("/home/DCCS5/cerebellum_project/src/stan/baseline_v7.stan", cpp_options = list(stan_threads = TRUE))

cat("Sampling Baseline V6 (N=30) ...\n")
fit_v7 <- mod_v7$sample(
  data = stan_data_v7,
  chains = 4,
  parallel_chains = 2,
  threads_per_chain = 4,
...
fit_v7$save_output_files(dir = "/home/DCCS5/cerebellum_project/results/", basename = "fit_v7_n30")

summ_v6 <- fit_v7$summary()
cat("Baseline V6 Diagnostics -> Max Rhat:", max(summ_v6$rhat, na.rm=TRUE), "| Min Bulk ESS:", min(summ_v6$ess_bulk, na.rm=TRUE), "\n")
loo_v7 <- fit_v7$loo()
saveRDS(loo_v7, "/home/DCCS5/cerebellum_project/results/loo_v7_n30.rds")

loo_m012 <- readRDS("/home/DCCS5/cerebellum_project/results/loo_m012_n30.rds")
cat("\n==========================================\n")
cat(" LOO-CV COMPARISON (N=30)\n")
cat("==========================================\n")
comp <- loo_compare(list(M012 = loo_m012, BaselineV7 = loo_v7))
print(comp)
cat("\n")
