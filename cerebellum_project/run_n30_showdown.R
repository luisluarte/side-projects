...
cat("==========================================\n")
cat(" FINAL N=30 SHOWDOWN: M012 vs BASELINE V2\n")
cat("==========================================\n\n")
...
# Save RAW output files directly so they aren't deleted by garbage collector!
fit_m012$save_output_files(dir = "/home/DCCS5/cerebellum_project/results/", basename = "fit_m012_n30")

# Diagnostics M012
summ_m012 <- fit_m012$summary()
max_rhat_m012 <- max(summ_m012$rhat, na.rm=TRUE)
min_ess_m012 <- min(summ_m012$ess_bulk, na.rm=TRUE)
cat("M012 Diagnostics -> Max Rhat:", max_rhat_m012, "| Min Bulk ESS:", min_ess_m012, "\n")

# LOO M012
cat("Extracting LOO for M012...\n")
loo_m012 <- fit_m012$loo()
saveRDS(loo_m012, "/home/DCCS5/cerebellum_project/results/loo_m012_n30.rds")

# ------------------------------------------------------------------------------
# 3. RUN BASELINE V2
# ------------------------------------------------------------------------------
# Note: stan_data for Baseline V2 requires fewer parameters in theta_mean.
# We will construct it locally.
stan_data_v2 <- stan_data
stan_data_v2$theta_mean <- rep(0, 8)
stan_data_v2$L_Sigma <- diag(8)

cat("\nCompiling Baseline V2...\n")
mod_v2 <- cmdstan_model("/home/DCCS5/cerebellum_project/src/stan/m008_baseline_v2.stan", cpp_options = list(stan_threads = TRUE))
...
fit_v2$save_output_files(dir = "/home/DCCS5/cerebellum_project/results/", basename = "fit_v2_n30")

# Diagnostics V2
summ_v2 <- fit_v2$summary()
max_rhat_v2 <- max(summ_v2$rhat, na.rm=TRUE)
min_ess_v2 <- min(summ_v2$ess_bulk, na.rm=TRUE)
cat("Baseline V2 Diagnostics -> Max Rhat:", max_rhat_v2, "| Min Bulk ESS:", min_ess_v2, "\n")

cat("Extracting LOO for Baseline V2...\n")
loo_v2 <- fit_v2$loo()
saveRDS(loo_v2, "/home/DCCS5/cerebellum_project/results/loo_v2_n30.rds")
...
# ------------------------------------------------------------------------------
# 5. GENERATIVE METRICS FOR M012
# ------------------------------------------------------------------------------
cat("\nCalculating Generative Predictive Metrics (ROC, PR, RT-RMSE, LMER)...\n")
# Calculate from the medians of M012
...
tnd_cap <- pmin(stan_data$min_rt - 0.05, 3.69)
