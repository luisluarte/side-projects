...
Rcpp::sourceCpp("src/models/epoch4_landscape.cpp")
...
writeLines("\n## Epoch: 4 | 2026-08-25\n\n### 1. MAGI Consensus (Post-Epoch Deliberation)\n*   **Melchior:** We must abandon modifying the continuous evidence accumulation ($v, a, w$) with the Cerebellum. Biologically, the Cerebellum projects to the Primary Motor Cortex. It can pre-activate motor pathways when certainty is high, effectively shortening the pure mechanical execution time.\n*   **Caspar:** This is mathematically brilliant. Modulating $t_{nd}$ does not interact with the Wiener density integral for the evidence accumulation phase. Therefore, the Striatum can completely control the variance ($v, a$) without contradiction, while the Cerebellum sharpens the distribution offset ($t_{nd}$).\n*   **Balthazar:** L4 defined: Topology 10 (Cerebellar Motor-Gating). Base=Baseline5 (10 param), Cand=Topo10 (11 param).\n\n### 2. The Abstract Landscape Evaluated\n*   **Biological Topology:** Topology 10 (Cerebellar Motor-Gating)\n*   **Equivalent Baseline:** Baseline 5 (Abstract Reward Integrator)\n\n### 3. Epoch Results Summary\n", fileConn)
close(fileConn)
...
for(var_idx in 1:10) {
    ...
    cand_eval <- mclapply(1:S, function(s_idx) {
        ...
        obj <- function(phi) {
            ll <- eval_topo_10(phi, genes, resp, out, rt, iti, f_dur)
            if(is.na(ll) || is.infinite(ll)) return(1e9)
            return(ll + lambda * sum(phi^2))
        }
        res <- optim(rep(0, 11), obj, method="L-BFGS-B", lower=rep(-5, 11), upper=rep(5, 11), control=list(maxit=30))
        rt_pred <- extract_topo_10(res$par, genes, resp, out, rt, iti, f_dur, FALSE)
        list(ll = res$value, rt_pred = rt_pred, trials = nrow(d), par=res$par)
    }, mc.cores = num_cores)
    ...
    obj1 <- function(phi) eval_topo_10(phi, genes, d1$Boundary+1, d1$`F`, d1$RT, d1$ITI, d1$F_dur)
    ...
    for(k in 1:20) {
        p_rand <- runif(11, -2, 2)
        rts <- extract_topo_10(p_rand, genes, d1$Boundary+1, d1$`F`, d1$RT, d1$ITI, d1$F_dur, FALSE)
    ...
    if(is_success && !is.na(Z_stat) && Z_stat < best_z) {
        best_z <- Z_stat
        best_name <- sprintf("MODEL_V%d_EPOCH4_BEST", var_idx)
    }
...
writeLines("*   **File Location:** `/src/models/epoch4_landscape.cpp`\n", fileConn)
