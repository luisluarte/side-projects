...
cat("Starting Sensitivity Analysis for Fatigue N_MF using", num_cores, "cores...\n")
...
for (n_mf in nodes) {
  ...
  results <- mclapply(1:S, function(s_idx) {
    ...
    obj_fg <- function(phi) { return(eval_bvk_full_gating_dist_fatigue(phi, resp, out, rt, iti, f_dur, n_mf) + lambda * sum(abs(phi))) }
    res <- optim(rep(0, 11), obj_fg, method="L-BFGS-B", lower=rep(-5, 11), upper=rep(5, 11))
    ll_val <- eval_bvk_full_gating_dist_fatigue(res$par, resp, out, rt, iti, f_dur, n_mf)
...
cat("\nSensitivity Results (Fatigue Distributed Model):\n")
print(sensitivity_res)
cat("Optimal N_MF =", best_N, "with Deviance =", best_ll, "\n\n")

# ----------------------------------------------------
# Phase 2: Final Model Comparison (QL Poly vs Optimal FG Fatigue)
# ----------------------------------------------------
res_df <- data.frame()
...
results_final <- mclapply(1:S, function(s_idx) {
    ...
    pred_rt_fg_fatigue <- extract_rt_bvk_full_gating_dist_fatigue(best_par_list[[s_idx]], resp, out, rt, iti, f_dur, best_N)
...
cat("==========================================\n")
res_df$Model <- factor(res_df$Model, levels=c("Dynamic QL Poly", paste0("Full Gating Fatigue (N=", best_N, ")")))
...
p <- ggplot(decile_df, aes(x = mean_log_emp, y = mean_log_pred, color = Model, group = Model)) +
  ...
  labs(title = paste0("Vincentized Calibration: QL Poly vs Full Gating Fatigue (N=", best_N, ")"),
       x = "Empirical log(RT) (Decile Means)",
       y = "Predicted log(RT) (Decile Means)",
       color = "Model") +
  theme_minimal() +
  scale_color_manual(values = setNames(c("#D55E00", "#E69F00"), c("Dynamic QL Poly", paste0("Full Gating Fatigue (N=", best_N, ")")))) +
  theme(plot.title = element_text(face = "bold", size = 14),
        legend.position = "bottom")

ggsave("fatigue_sens_rt_decile_plot.pdf", p, width = 8, height = 6)
ggsave("fatigue_sens_rt_decile_plot.png", p, width = 8, height = 6, dpi=300)
