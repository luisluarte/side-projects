...
List evaluate_eccm_70_30_cpp(
    ...
    const IntegerVector& is_test_R, bool return_test_metrics = false
) {
  int N_t = resp_R.size();
...
List evaluate_m1_70_30_cpp(
    const NumericVector& p, const IntegerVector& resp_R, const IntegerVector& out_R, const NumericVector& rt_R,
    const IntegerVector& subj_idx_R, const IntegerVector& is_test_R, bool return_test_metrics = false
) {
  double b_v=p[0], a_0=p[1], t_nd=p[2], train_nll=0.0, test_nll=0.0; int test_count=0, N_t=resp_R.size();
  for (int t=0; t<N_t; ++t) {
    int ch=resp_R[t], prev_ch=(t>0 && subj_idx_R[t]==subj_idx_R[t-1]) ? resp_R[t-1] : 1;
    int prev_out=(t>0 && subj_idx_R[t]==subj_idx_R[t-1]) ? out_R[t-1] : 1;
    int c_wsls=1; if (t>0 && subj_idx_R[t]==subj_idx_R[t-1]) c_wsls=(prev_out==1) ? prev_ch : ((prev_ch==1)?2:1);
    double v_t = b_v*((c_wsls==1)?1.0:-1.0);
    double dens = wiener_pdf(rt_R[t], ch, v_t, a_0, t_nd);
    if (is_test_R[t]==1) { test_nll -= std::log(dens); test_count++; } else { train_nll -= std::log(dens); }
  }
  if (return_test_metrics) return List::create(Named("Train_NLL")=train_nll, Named("Test_NLL")=test_nll, Named("Test_Count")=test_count);
  return List::create(Named("Obj")=train_nll);
}
...
List evaluate_m2_70_30_cpp(
  ...
  double Q_rw_cf[2] = {0.50, 0.50}, train_nll=0.0, test_nll=0.0; int test_count=0, N_t=resp_R.size();
  for (int t=0; t<N_t; ++t) {
    if (t>0 && subj_idx_R[t]!=subj_idx_R[t-1]) { Q_rw_cf[0]=0.50; Q_rw_cf[1]=0.50; }
    int ch=resp_R[t], out=out_R[t];
    double delta_rpe = (double)out - Q_rw_cf[ch-1];
    double dens = wiener_pdf(rt_R[t], ch, b_v*(Q_rw_cf[0]-Q_rw_cf[1]), std::max(0.30, a_0 + k_mod*std::abs(delta_rpe)), t_nd);
    if (is_test_R[t]==1) { test_nll -= std::log(dens); test_count++; } else { train_nll -= std::log(dens); }
    double c_alpha = (is_test_R[t]==1) ? 0.0 : alpha_q;
    Q_rw_cf[ch-1] += c_alpha*delta_rpe; Q_rw_cf[(ch==1)?1:0] += c_alpha*((1.0-(double)out)-Q_rw_cf[(ch==1)?1:0]);
  }
  if (return_test_metrics) return List::create(Named("Train_NLL")=train_nll, Named("Test_NLL")=test_nll, Named("Test_Count")=test_count);
  return List::create(Named("Obj")=train_nll);
}
'
sourceCpp(code = cpp_eccm)

# --- RE-OPTIMIZE M1 & M2 QUICKLY (Since we did not save weights) ---
m1_lower <- c(0.01, 0.40, 0.08); m1_upper <- c(3.50, 2.50, 0.30); m1_init <- c(0.80, 1.20, 0.18)
obj_m1 <- function(p) { if(any(p<m1_lower)||any(p>m1_upper)) return(1e9); evaluate_m1_70_30_cpp(p, as.integer(dat_all$Resp), as.integer(dat_all$F), dat_all$RT, as.integer(as.factor(dat_all$participant_id)), dat_all$is_test, FALSE)$Obj }
m1_opt <- optim(m1_init, obj_m1, method="L-BFGS-B", lower=m1_lower, upper=m1_upper, control=list(maxit=30))$par

m2_lower <- c(0.01, 0.01, 0.40, 0.00, 0.08); m2_upper <- c(0.90, 3.50, 2.50, 0.50, 0.30); m2_init <- c(0.15, 0.80, 1.20, 0.05, 0.18)
obj_m2 <- function(p) { if(any(p<m2_lower)||any(p>m2_upper)) return(1e9); evaluate_m2_70_30_cpp(p, as.integer(dat_all$Resp), as.integer(dat_all$F), dat_all$RT, as.integer(as.factor(dat_all$participant_id)), dat_all$is_test, FALSE)$Obj }
m2_opt <- optim(m2_init, obj_m2, method="L-BFGS-B", lower=m2_lower, upper=m2_upper, control=list(maxit=30))$par

eccm_opt <- c(0.77625624, 1.46237281, 0.12790352, 0.42522427, 0.55880863, 1.98470561, 2.28730897, 0.35895200, 0.91465712, 0.31305780, 0.06394818, 0.23745569, 0.24399459)

# --- CALCULATE NLL PER PARTICIPANT ---
res_df <- data.frame(Subject=character(), M1_NLL_per_trial=numeric(), M2_NLL_per_trial=numeric(), ECCM_NLL_per_trial=numeric(), stringsAsFactors=FALSE)

for (i in 1:length(sample_participants)) {
    p <- sample_participants[i]
    sub_dat <- dat_all[dat_all$participant_id == p, ]
    
    r_m1 <- evaluate_m1_70_30_cpp(m1_opt, as.integer(sub_dat$Resp), as.integer(sub_dat$F), sub_dat$RT, rep(1, nrow(sub_dat)), sub_dat$is_test, TRUE)
    r_m2 <- evaluate_m2_70_30_cpp(m2_opt, as.integer(sub_dat$Resp), as.integer(sub_dat$F), sub_dat$RT, rep(1, nrow(sub_dat)), sub_dat$is_test, TRUE)
    r_eccm <- evaluate_eccm_70_30_cpp(eccm_opt, as.integer(sub_dat$Resp), as.integer(sub_dat$F), sub_dat$Bd1, sub_dat$Bd2, sub_dat$RT, as.numeric(sub_dat$ttp), as.numeric(sub_dat$ttF), rep(1, nrow(sub_dat)), sub_dat$is_test, TRUE)
    
    if (r_m1$Test_Count > 0) {
        res_df <- rbind(res_df, data.frame(
            Subject = p,
            M1_NLL_per_trial = r_m1$Test_NLL / r_m1$Test_Count,
            M2_NLL_per_trial = r_m2$Test_NLL / r_m2$Test_Count,
            ECCM_NLL_per_trial = r_eccm$Test_NLL / r_eccm$Test_Count
        ))
    }
}

cat("\\n=== Per-Subject 30% Test Set NLL / Trial ===\\n")
print(res_df)

cat("\\n=== Paired T-Tests (ECCM vs Baseline) ===\\n")
t1 <- t.test(res_df$ECCM_NLL_per_trial, res_df$M1_NLL_per_trial, paired=TRUE, alternative="two.sided")
t2 <- t.test(res_df$ECCM_NLL_per_trial, res_df$M2_NLL_per_trial, paired=TRUE, alternative="two.sided")

cat("ECCM vs M1 (WSLS):\\n")
cat(sprintf("  Mean Diff: %.4f, t(%d) = %.4f, p = %.4e\\n", t1$estimate, t1$parameter, t1$statistic, t1$p.value))

cat("\\nECCM vs M2 (RWCF):\\n")
cat(sprintf("  Mean Diff: %.4f, t(%d) = %.4f, p = %.4e\\n", t2$estimate, t2$parameter, t2$statistic, t2$p.value))
