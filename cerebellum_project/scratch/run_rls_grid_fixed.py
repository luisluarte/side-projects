...
for (n_comp in n_comp_list) {
    ...
    for (fold in 1:10) {
        ...
        obj_fun <- function(phi) {
            evaluate_rls_cmaes(phi, as.integer(tr_dat$Resp), as.integer(tr_dat$F), tr_dat$RT, as.numeric(tr_dat$ttp), as.integer(as.factor(tr_dat$participant_id)), n_comp)
        }
        
        res <- cma_es(initial_phi, obj_fun, lower = lower_bounds, upper = upper_bounds, control = list(maxit = 50, stopfitness = -Inf))
        
        ts_nll <- evaluate_rls_cmaes(res$par, as.integer(ts_dat$Resp), as.integer(ts_dat$F), ts_dat$RT, as.numeric(ts_dat$ttp), rep(1, nrow(ts_dat)), n_comp)
...
with open("rls_compression_grid.R", "w") as f:
    f.write(r_script_content)

print("R script for RLS created.")
subprocess.run(["Rscript", "rls_compression_grid.R"], check=True)
