...
# Mark the 70/30 split per participant
dat_all$is_test <- 0
...
List evaluate_eccm_70_30_cpp(
  ...
  double beta_v      = phi_13d[0];
  double a_0         = phi_13d[1];
  double t_nd        = phi_13d[2];
  double kappa_a     = phi_13d[3];
  double mu_beta     = phi_13d[4];
  double sigma_beta  = phi_13d[5];
  double lambda_d    = phi_13d[6];
  double mu_tau      = phi_13d[7];
  double sigma_tau   = phi_13d[8];
  double rho_base    = phi_13d[9];
  double eta         = phi_13d[10];
  double lambda      = phi_13d[11];
  double theta_th    = phi_13d[12];
  ...
  for (int k = 0; k < N_MLI; ++k) {
      for (int i = 0; i < N_GC; ++i) {
          W_GC_MLI[k][i] = rng.runif() * theta_max;
      }
  }
  ...
  for (int t = 0; t < N_t; ++t) {
    ...
    state_hist[0][0] = (prev_ch == 1) ? 1.0 : -1.0;
    state_hist[0][1] = (prev_out == 1) ? 1.0 : -1.0;
    state_hist[0][2] = d_curr;
    state_hist[0][3] = d_diff;
    state_hist[0][4] = std::max(-2.0, std::min(2.0, (prev_rt - 0.75) / 0.50));
    state_hist[0][5] = std::max(-2.0, std::min(2.0, (prev_iti - 7.0) / 3.0));

    // Phase 1: Mossy Fibers
    ...
    // Phase 2: Granular Candidate Manifold
    for (int i = 0; i < N_GC; ++i) {
        double in_sum = 0.0;
    ...
    // Phase 3: MLI Spatial Pooling
    std::vector<double> h_MLI(N_MLI, 0.0);
    double l1_mli_sum = 1e-12;
    for (int k = 0; k < N_MLI; ++k) {
        double pool_sum = 0.0;
        for (int i = 0; i < N_GC; ++i) {
            pool_sum += W_GC_MLI[k][i] * z_GC_curr[i];
        }
        h_MLI[k] = std::max(0.0, pool_sum - theta_th);
        l1_mli_sum += h_MLI[k];
    }
    
    // Phase 5 Entropy computation
    double S_MLI = 0.0;
    for (int k = 0; k < N_MLI; ++k) {
        double pk = h_MLI[k] / l1_mli_sum;
        if (pk > 1e-12) S_MLI -= pk * std::log(pk);
    }
    double max_S = std::log((double)N_MLI);
    double norm_S = S_MLI / max_S;

    // Phase 4: Purkinje Readout
    double y_PC1 = 0.0, y_PC2 = 0.0;
    for (int k = 0; k < N_MLI; ++k) {
        y_PC1 += W_PF1[k] * h_MLI[k];
        y_PC2 += W_PF2[k] * h_MLI[k];
    }

    // Phase 5: HDDM Integration
    ...
    // IO Error (mapped OUT to physical magnitude +/- 1)
    double target = ((double)out - 0.5) * 2.0;
    double y_chosen = (ch == 1) ? y_PC1 : y_PC2;
    double delta_IO = target - y_chosen;

    // Plasticity (LOCKED IF TEST)
    double current_eta = is_test ? 0.0 : eta;
    double current_lambda = is_test ? 0.0 : lambda;

    for (int k = 0; k < N_MLI; ++k) {
        if (ch == 1) {
            W_PF1[k] += current_eta * delta_IO * h_MLI[k] - current_lambda * W_PF1[k];
            W_PF2[k] += -current_lambda * W_PF2[k];
        } else {
            W_PF2[k] += current_eta * delta_IO * h_MLI[k] - current_lambda * W_PF2[k];
            W_PF1[k] += -current_lambda * W_PF1[k];
        }
    }
  ...
  if (return_test_metrics) {
      return List::create(
          Named("Train_NLL") = train_nll,
          Named("Test_NLL") = test_nll,
          Named("Test_Count") = test_count
      );
  } else {
...
obj_fun <- function(phi) {
    ...
    res <- evaluate_eccm_70_30_cpp(
        phi, 
        as.integer(dat_all$Resp), 
        as.integer(dat_all$F), 
        dat_all$Bd1, dat_all$Bd2, 
        dat_all$RT, as.numeric(dat_all$ttp), as.numeric(dat_all$ttF), 
        as.integer(as.factor(dat_all$participant_id)), 
        dat_all$is_test, 
        FALSE
    )
    return(res$Obj)
...
cat("\\n=== Running CMA-ES on 70% Global Train Split ===\\n")
# CMA-ES on the training split ONLY. (is_test == 1 inside C++ returns Train_NLL exclusively)
cma_res <- cma_es(initial_phi, obj_fun, lower = lower_bounds, upper = upper_bounds, control = list(maxit = 50, trace = TRUE, sigma = 0.2))

phi_opt <- cma_res$par
cat("\\nOptimal Hyperparameters:\\n")
print(phi_opt)

cat("\\n=== Evaluating 30% Test Split ===\\n")
final_res <- evaluate_eccm_70_30_cpp(
    phi_opt, 
    as.integer(dat_all$Resp), 
    as.integer(dat_all$F), 
    dat_all$Bd1, dat_all$Bd2, 
...
cat(sprintf("Train NLL: %.2f\\n", final_res$Train_NLL))
cat(sprintf("Test NLL:  %.2f (over %d trials)\\n", final_res$Test_NLL, final_res$Test_Count))

write.csv(data.frame(Train_NLL=final_res$Train_NLL, Test_NLL=final_res$Test_NLL, Test_Count=final_res$Test_Count), "results/tables/eccm_70_30_results.csv", row.names=FALSE)
cat("\\nDone!\\n")
