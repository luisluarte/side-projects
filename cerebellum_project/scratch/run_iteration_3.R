...
cat("Loading and partitioning N=30 dataset for Iteration 3...\\n")
...
// [[Rcpp::export]]
NumericVector evaluate_eccm_cv_dynamic_iter3(
  ...
  double gamma_e = phi_16d[15]; // NEW: Eligibility trace decay
  ...
  for (int t=0; t<N_t; ++t) {
    ...
    for (int k=0; k<N_MLI; ++k) {
        ...
        l1_mli_sum += h_MLI[k];
        e_trace[k] = gamma_e * e_trace[k] + h_MLI[k]; // Update trace
    }
...
# ================= Combined ECCM + AntiCorr + Traces (20:200:40) ================= #
cat("Training Combined ECCM (MF=20, GC=200, MLI=40, AntiCorr=1, Traces)...\\n")
# Added 16th parameter: gamma_e
lower_bounds <- c(b_v = 0.0, a_0 = 0.30, t_nd = 0.10, kappa_a = 0.0, mu_beta = -2.0, sigma_beta = 0.01, lambda_d = 0.0, mu_tau = -2.0, sigma_tau = 0.01, rho_base = 0.0, eta = 0.0, lambda = 0.0, theta_th = 0.0, alpha_goc = 0.0, kappa_th = 0.0, gamma_e = 0.0)
upper_bounds <- c(b_v = 3.0, a_0 = 2.50, t_nd = 0.90, kappa_a = 2.0, mu_beta =  2.0, sigma_beta = 2.00, lambda_d = 5.0, mu_tau =  2.0, sigma_tau = 2.00, rho_base = 0.95, eta = 1.0, lambda = 0.5, theta_th = 0.5, alpha_goc = 2.0, kappa_th = 2.0, gamma_e = 0.95)
...
obj_eccm <- function(phi) {
    ...
    res <- evaluate_eccm_cv_dynamic_iter3(phi, as.integer(dat_all$Resp), as.integer(dat_all$F), dat_all$Bd1, dat_all$Bd2, dat_all$RT, as.numeric(dat_all$ttp), as.numeric(dat_all$ttF), dat_all$participant_factor, as.integer(dat_all$is_test), 1, 1, 1, 20, 200, 40)
    return(res[1]) 
}
cma_eccm <- cma_es(initial_phi, obj_eccm, lower = lower_bounds, upper = upper_bounds, control = list(maxit = 35, trace = TRUE, sigma = 0.2))
results_df$ECCM_Iter3_NLL <- evaluate_eccm_cv_dynamic_iter3(cma_eccm$par, as.integer(dat_all$Resp), as.integer(dat_all$F), dat_all$Bd1, dat_all$Bd2, dat_all$RT, as.numeric(dat_all$ttp), as.numeric(dat_all$ttF), dat_all$participant_factor, as.integer(dat_all$is_test), 1, 1, 1, 20, 200, 40)[2:31]
...
cat(sprintf("ECCM Iter3: %.2f\\n", sum(results_df$ECCM_Iter3_NLL)))

cat("\\n--- PAIRED T-TESTS (Participant Level) ---\\n")
t1 <- t.test(results_df$M1_NLL, results_df$ECCM_Iter3_NLL, paired=TRUE)
cat(sprintf("M1 vs ECCM3: t = %.2f, p = %.3e (Mean Diff: %.2f)\\n", t1$statistic, t1$p.value, t1$estimate))

t2 <- t.test(results_df$M2_NLL, results_df$ECCM_Iter3_NLL, paired=TRUE)
cat(sprintf("M2 vs ECCM3: t = %.2f, p = %.3e (Mean Diff: %.2f)\\n", t2$statistic, t2$p.value, t2$estimate))

write.csv(results_df, "results/tables/iteration_3_n30.csv", row.names=FALSE)
