...
List evaluate_eccm_generalized_cpp(
    ...
    const IntegerVector& is_test_R, int N_MF, int N_GC, int N_MLI, IntegerVector active_features, 
...
run_optimization <- function(n_mf, n_gc, n_mli, max_iter=20) {
    obj_fun <- function(phi) {
        ...
        res <- evaluate_eccm_generalized_cpp(
            ...
            n_mf, n_gc, n_mli, all_feat, FALSE
        )
        return(res$Obj)
    }
    ...
    final_res <- evaluate_eccm_generalized_cpp(
        ...
        dat_all$RT, as.numeric(dat_all$ttp), as.numeric(dat_all$ttF), 
        as.integer(as.factor(dat_all$participant_id)), dat_all$is_test, 
        n_mf, n_gc, n_mli, all_feat, TRUE
    )
    return(list(train_nll=final_res$Train_NLL, test_nll=final_res$Test_NLL, test_count=final_res$Test_Count))
}

cat("\\n===========================================\\n")
cat("STARTING 3D RATIO OPTIMIZATION (MF x GC x MLI)\\n")
cat("===========================================\\n")

mf_grid <- c(10, 20, 40)
mli_grid <- c(10, 20, 40, 80)
gc_grid <- c(100, 200, 400, 800)

results_df <- data.frame(MF=integer(), GC=integer(), MLI=integer(), Train_NLL=numeric(), Test_NLL=numeric())
best_test <- 1e9
best_combo <- ""
...
for (mf in mf_grid) {
  for (mli in mli_grid) {
    for (gc in gc_grid) {
      if (gc > mf && gc > mli) {
        cat(sprintf("Testing [MF=%02d | GC=%03d | MLI=%02d]... ", mf, gc, mli))
        res <- run_optimization(mf, gc, mli, max_iter=20)
        cat(sprintf("Test NLL: %.2f\\n", res$test_nll))
        
        results_df <- rbind(results_df, data.frame(MF=mf, GC=gc, MLI=mli, Train_NLL=res$train_nll, Test_NLL=res$test_nll))
        
        if (res$test_nll < best_test) {
          best_test <- res$test_nll
          best_combo <- sprintf("MF=%d, GC=%d, MLI=%d", mf, gc, mli)
        }
      }
    }
  }
...
cat(sprintf("Absolute Best Configuration: [%s] -> Test NLL = %.2f\\n", best_combo, best_test))

write.csv(results_df, "results/tables/eccm_ratio_optimization.csv", row.names=FALSE)
