...
cat("Loading and partitioning N=30 dataset for Iteration 2...\\n")
...
// [[Rcpp::export]]
NumericVector evaluate_eccm_cv_dynamic_16d(
    const NumericVector& phi_16d, const IntegerVector& resp_R, const IntegerVector& out_R,
  ...
  int N_t = resp_R.size();
  double beta_v=phi_16d[0], a_0=phi_16d[1], t_nd=phi_16d[2], kappa_a=phi_16d[3], mu_beta=phi_16d[4], sigma_beta=phi_16d[5];
  double lambda_d=phi_16d[6], mu_tau=phi_16d[7], sigma_tau=phi_16d[8], rho_base=phi_16d[9], eta=phi_16d[10], lambda=phi_16d[11], theta_th=phi_16d[12];
  double alpha_goc = phi_16d[13];
  double kappa_th = phi_16d[14];
  double rho_eta = phi_16d[15]; // NEW: Asymmetric learning rate ratio
  ...
  for (int t=0; t<N_t; ++t) {
    ...
    if (is_test_R[t] == 0) {
        ...
        if (use_anticorr == 1) {
            ...
            for (int k=0; k<N_MLI; ++k) {
                double eta_1 = (delta_IO_1 > 0) ? (eta * rho_eta) : eta;
                double eta_2 = (delta_IO_2 > 0) ? (eta * rho_eta) : eta;
                W_PF1[k] += eta_1*delta_IO_1*h_MLI[k] - lambda*W_PF1[k];
                W_PF2[k] += eta_2*delta_IO_2*h_MLI[k] - lambda*W_PF2[k];
            }
        } else {
...
# We will read Iteration 1 results to reuse M1 and M2 baselines instead of running them again!
...
# ================= Combined ECCM + AntiCorr + AsymmEta (20:200:40) ================= #
cat("Training Combined ECCM (MF=20, GC=200, MLI=40, AntiCorr=1, AsymmEta)...\\n")
# Added 16th parameter: rho_eta
lower_bounds <- c(b_v = 0.0, a_0 = 0.30, t_nd = 0.10, kappa_a = 0.0, mu_beta = -2.0, sigma_beta = 0.01, lambda_d = 0.0, mu_tau = -2.0, sigma_tau = 0.01, rho_base = 0.0, eta = 0.0, lambda = 0.0, theta_th = 0.0, alpha_goc = 0.0, kappa_th = 0.0, rho_eta = 0.1)
upper_bounds <- c(b_v = 3.0, a_0 = 2.50, t_nd = 0.90, kappa_a = 2.0, mu_beta =  2.0, sigma_beta = 2.00, lambda_d = 5.0, mu_tau =  2.0, sigma_tau = 2.00, rho_base = 0.95, eta = 1.0, lambda = 0.5, theta_th = 0.5, alpha_goc = 2.0, kappa_th = 2.0, rho_eta = 10.0)
...
obj_eccm <- function(phi) {
    ...
    res <- evaluate_eccm_cv_dynamic_16d(phi, as.integer(dat_all$Resp), as.integer(dat_all$F), dat_all$Bd1, dat_all$Bd2, dat_all$RT, as.numeric(dat_all$ttp), as.numeric(dat_all$ttF), dat_all$participant_factor, as.integer(dat_all$is_test), 1, 1, 1, 20, 200, 40)
    return(res[1]) 
}
cma_eccm <- cma_es(initial_phi, obj_eccm, lower = lower_bounds, upper = upper_bounds, control = list(maxit = 35, trace = TRUE, sigma = 0.2))
results_df$ECCM_Iter2_NLL <- evaluate_eccm_cv_dynamic_16d(cma_eccm$par, as.integer(dat_all$Resp), as.integer(dat_all$F), dat_all$Bd1, dat_all$Bd2, dat_all$RT, as.numeric(dat_all$ttp), as.numeric(dat_all$ttF), dat_all$participant_factor, as.integer(dat_all$is_test), 1, 1, 1, 20, 200, 40)[2:31]
...
cat(sprintf("ECCM Iter2: %.2f\\n", sum(results_df$ECCM_Iter2_NLL)))

cat("\\n--- PAIRED T-TESTS (Participant Level) ---\\n")
t1 <- t.test(results_df$M1_NLL, results_df$ECCM_Iter2_NLL, paired=TRUE)
cat(sprintf("M1 vs ECCM2: t = %.2f, p = %.3e (Mean Diff: %.2f)\\n", t1$statistic, t1$p.value, t1$estimate))

t2 <- t.test(results_df$M2_NLL, results_df$ECCM_Iter2_NLL, paired=TRUE)
cat(sprintf("M2 vs ECCM2: t = %.2f, p = %.3e (Mean Diff: %.2f)\\n", t2$statistic, t2$p.value, t2$estimate))

write.csv(results_df, "results/tables/iteration_2_n30.csv", row.names=FALSE)
