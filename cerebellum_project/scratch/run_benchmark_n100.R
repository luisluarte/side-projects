...
cat("Loading and partitioning N=100 dataset...\\n")
...
set.seed(999) 
sample_participants <- sample(unique(dat_all[['participant_id']]), 100)
...
NumericVector evaluate_m1_cv_cpp(const NumericVector& phi, const IntegerVector& resp, const IntegerVector& is_test, const NumericVector& rt, const IntegerVector& subj_idx) {
    ...
    std::vector<double> test_nlls(101, 0.0);
...
NumericVector evaluate_eccm_cv_dynamic(
  ...
  for (int t=0; t<N_t; ++t) {
    ...
    if (is_test_R[t] == 0) {
        ...
        if(subj >= 1 && subj <= 100) test_nlls[subj] += nll_t;
    }
...
results_df <- data.frame(Participant = 1:100)
...
results_df$M1_NLL <- evaluate_m1_cv_cpp(cma_m1$par, as.integer(dat_all$Resp), as.integer(dat_all$is_test), dat_all$RT, dat_all$participant_factor)[2:101]
...
results_df$M2_NLL <- evaluate_m2_cv_cpp(cma_m2$par, as.integer(dat_all$Resp), as.integer(dat_all$F), as.integer(dat_all$is_test), dat_all$RT, dat_all$participant_factor)[2:101]

# ================= Combined ECCM (40:500:80) ================= #
cat("Training Combined ECCM (MF=40, GC=500, MLI=80)...\\n")
...
obj_eccm <- function(phi) {
    ...
    res <- evaluate_eccm_cv_dynamic(phi, as.integer(dat_all$Resp), as.integer(dat_all$F), dat_all$Bd1, dat_all$Bd2, dat_all$RT, as.numeric(dat_all$ttp), as.numeric(dat_all$ttF), dat_all$participant_factor, as.integer(dat_all$is_test), 1, 1, 40, 500, 80)
    return(res[1]) 
}
# Set sigma slightly lower because high dimensions can destabilize search
cma_eccm <- cma_es(initial_phi, obj_eccm, lower = lower_bounds, upper = upper_bounds, control = list(maxit = 35, trace = TRUE, sigma = 0.15))
results_df$Combined_NLL <- evaluate_eccm_cv_dynamic(cma_eccm$par, as.integer(dat_all$Resp), as.integer(dat_all$F), dat_all$Bd1, dat_all$Bd2, dat_all$RT, as.numeric(dat_all$ttp), as.numeric(dat_all$ttF), dat_all$participant_factor, as.integer(dat_all$is_test), 1, 1, 40, 500, 80)[2:101]

cat("\\n======================================\\n")
cat("AGGREGATE TEST NLL ACROSS 100 PARTICIPANTS\\n")
...
write.csv(results_df, "results/tables/eccm_n100_benchmark_500.csv", row.names=FALSE)
