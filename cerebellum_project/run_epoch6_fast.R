...
Rcpp::sourceCpp("src/models/epoch6_landscape.cpp")
...
writeLines("\n## Epoch: 6 | 2026-08-25\n\n### 1. MAGI Consensus (Post-Epoch Deliberation)\n*   **Melchior:** Injecting the Cerebellar temporal prediction directly into the LBA drift rate failed. The LBA assumes ballistic, non-volatile evidence accumulation. We must find an orthogonal axis for the Cerebellum.\n*   **Caspar:** The LBA has a unique parameter: $A$, the starting point variance. Biologically, the Cerebellum provides precision and variance reduction. If the Cerebellar forward model is highly confident, it should clamp the starting point uncertainty ($A \\to 0$).\n*   **Balthazar:** L6 defined: Topology 12 (Cerebellar Precision Gating). Cortex drives drift ($v$), Striatum drives the threshold fatigue ($b$), and Cerebellum dynamically clamps the starting point variance ($A$). Base=Baseline LBA (8 param), Cand=Topo12 (9 param).\n\n### 2. The Abstract Landscape Evaluated\n*   **Biological Topology:** Topology 12 (Cerebellar Precision Gating)\n*   **Equivalent Baseline:** Baseline LBA (Abstract Reward Integrator)\n\n### 3. Epoch Results Summary\n", fileConn)
close(fileConn)
...
for(var_idx in 1:10) {
    ...
    cand_eval <- mclapply(1:S, function(s_idx) {
        ...
        obj <- function(phi) {
            ll <- eval_topo_12(phi, genes, resp, out, rt, iti, f_dur)
            if(is.na(ll) || is.infinite(ll)) return(1e9)
            return(ll + lambda * sum(phi^2))
        }
        res <- optim(rep(0, 9), obj, method="L-BFGS-B", lower=rep(-5, 9), upper=rep(5, 9), control=list(maxit=30))
        rt_pred <- extract_topo_12(res$par, genes, resp, out, rt, iti, f_dur, FALSE)
        list(ll = res$value, rt_pred = rt_pred, trials = nrow(d), par=res$par)
    }, mc.cores = num_cores)
    ...
    obj1 <- function(phi) eval_topo_12(phi, genes, d1$Boundary+1, d1$`F`, d1$RT, d1$ITI, d1$F_dur)
    ...
    for(k in 1:20) {
        p_rand <- runif(9, -2, 2)
        rts <- extract_topo_12(p_rand, genes, d1$Boundary+1, d1$`F`, d1$RT, d1$ITI, d1$F_dur, FALSE)
    ...
    if(is_success && !is.na(Z_stat) && Z_stat < best_z) {
        best_z <- Z_stat
        best_name <- sprintf("MODEL_V%d_EPOCH6_BEST", var_idx)
    }
...
writeLines("*   **File Location:** `/src/models/epoch6_landscape.cpp`\n", fileConn)
