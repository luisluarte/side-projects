...
cat("Fitting 3 Models and Extracting Expected RTs...\n")
res_df <- data.frame()
...
results_final <- mclapply(1:S, function(s_idx) {
    ...
    # Model 2: Dynamic QL Poly Fatigue
    obj_dyn_poly_fatigue <- function(phi) { return(eval_ql_ddm_dynamic_poly_fatigue(phi, resp, out, rt) + lambda * sum(abs(phi))) }
    res_dyn_poly_fatigue <- optim(rep(0, 9), obj_dyn_poly_fatigue, method="L-BFGS-B", lower=rep(-5, 9), upper=rep(5, 9))
    pred_rt_dyn_poly_fatigue <- extract_rt_ql_dynamic_poly_fatigue(res_dyn_poly_fatigue$par, resp, out, rt)
    
    # Model 3: Full Gating Fatigue (N=20)
    ...
    df2 <- data.frame(Participant = d$participant_id, Trial = d$ttp, RT_empirical = rt, Model = "Dynamic QL Poly Fatigue", RT_prediction = pred_rt_dyn_poly_fatigue)
    df3 <- data.frame(Participant = d$participant_id, Trial = d$ttp, RT_empirical = rt, Model = paste0("Full Gating Fatigue (N=", best_N, ")"), RT_prediction = pred_rt_fg_fatigue)
...
cat("==========================================\n")
res_df$Model <- factor(res_df$Model, levels=c("Dynamic QL Poly Fatigue", "Dynamic QL Poly", paste0("Full Gating Fatigue (N=", best_N, ")")))
...
p <- ggplot(decile_df, aes(x = mean_log_emp, y = mean_log_pred, color = Model, group = Model)) +
  ...
  labs(title = "Vincentized Calibration: 3-Way Fatigue Comparison",
       x = "Empirical log(RT) (Decile Means)",
       y = "Predicted log(RT) (Decile Means)",
       color = "Model") +
  theme_minimal() +
  scale_color_manual(values = setNames(c("#009E73", "#D55E00", "#E69F00"), c("Dynamic QL Poly Fatigue", "Dynamic QL Poly", paste0("Full Gating Fatigue (N=", best_N, ")")))) +
  theme(plot.title = element_text(face = "bold", size = 14),
        legend.position = "bottom")

ggsave("final_3way_rt_decile_plot.pdf", p, width = 8, height = 6)
ggsave("final_3way_rt_decile_plot.png", p, width = 8, height = 6, dpi=300)
