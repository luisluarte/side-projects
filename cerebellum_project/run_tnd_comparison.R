...
cat("Fitting Models with Dynamic T_ND and Extracting Expected RTs...\n")
res_df <- data.frame()
...
results_final <- mclapply(1:S, function(s_idx) {
    ...
    obj_dyn_poly <- function(phi) { return(eval_ql_ddm_dynamic_poly_tnd(phi, resp, out, rt) + lambda * sum(abs(phi))) }
    res_dyn_poly <- optim(rep(0, 11), obj_dyn_poly, method="L-BFGS-B", lower=rep(-5, 11), upper=rep(5, 11))
    pred_rt_dyn_poly <- extract_rt_ql_dynamic_poly_tnd(res_dyn_poly$par, resp, out, rt)
    
    obj_fg_fatigue <- function(phi) { return(eval_bvk_full_gating_dist_fatigue_tnd(phi, resp, out, rt, iti, f_dur, best_N) + lambda * sum(abs(phi))) }
    res_fg_fatigue <- optim(rep(0, 14), obj_fg_fatigue, method="L-BFGS-B", lower=rep(-5, 14), upper=rep(5, 14))
    pred_rt_fg_fatigue <- extract_rt_bvk_full_gating_dist_fatigue_tnd(res_fg_fatigue$par, resp, out, rt, iti, f_dur, best_N)
    
    df1 <- data.frame(Participant = d$participant_id, Trial = d$ttp, RT_empirical = rt, Model = "Dynamic QL Poly TND", RT_prediction = pred_rt_dyn_poly)
    df2 <- data.frame(Participant = d$participant_id, Trial = d$ttp, RT_empirical = rt, Model = paste0("Full Gating Fatigue TND (N=", best_N, ")"), RT_prediction = pred_rt_fg_fatigue)
...
cat("==========================================\n")
res_df$Model <- factor(res_df$Model, levels=c("Dynamic QL Poly TND", paste0("Full Gating Fatigue TND (N=", best_N, ")")))
...
p <- ggplot(decile_df, aes(x = mean_log_emp, y = mean_log_pred, color = Model, group = Model)) +
  ...
  labs(title = paste0("Vincentized Calibration: QL Poly TND vs Full Gating TND (N=", best_N, ")"),
       x = "Empirical log(RT) (Decile Means)",
       y = "Predicted log(RT) (Decile Means)",
       color = "Model") +
  theme_minimal() +
  scale_color_manual(values = setNames(c("#D55E00", "#0072B2"), c("Dynamic QL Poly TND", paste0("Full Gating Fatigue TND (N=", best_N, ")")))) +
  theme(plot.title = element_text(face = "bold", size = 14),
        legend.position = "bottom")

ggsave("tnd_rt_decile_plot.pdf", p, width = 8, height = 6)
ggsave("tnd_rt_decile_plot.png", p, width = 8, height = 6, dpi=300)
