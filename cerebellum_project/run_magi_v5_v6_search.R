...
fileConn <- file("live_audit_log_v5_v6.md", "w")
writeLines("# MAGI Topology 5 & 6 (Golgi Integrator & Divisive Normalization) Search Ledger\n", fileConn)
...
for(var_idx in 1:20) {
    ...
    mask[17] <- 1
    mask[18] <- 1
    mask[19] <- 0 # We use 18 parameters total (phi 0-18 = 19 length, index 18 is 19th)
    # Wait, the C++ code uses phi[16], phi[17], phi[18]. So index 16, 17, 18. Mask length is 19.
    mask[17] <- 1; mask[18] <- 1; mask[19] <- 1; 
    ...
    fileConn <- file("live_audit_log_v5_v6.md", "a")
...
cat("Topology 5 and 6 Search Complete.\n")
