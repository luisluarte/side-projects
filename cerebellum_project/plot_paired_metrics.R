...
cat("Calculating metrics per participant...\n")
...
for(pid in unique(switch_df$participant_id)) {
  ...
  if(length(unique(sub_df$switch)) == 2) {
    ...
    pr_bvk <- pr.curve(scores.class0 = sub_df$p_switch_bvk[sub_df$switch==1], 
                       scores.class1 = sub_df$p_switch_bvk[sub_df$switch==0])$auc.integral
    ...
    pr_q <- pr.curve(scores.class0 = sub_df$p_switch_q[sub_df$switch==1], 
                     scores.class1 = sub_df$p_switch_q[sub_df$switch==0])$auc.integral
    mcc_q <- calculate_mcc(sub_df$switch, ifelse(sub_df$p_switch_q > 0.5, 1, 0))
    
    results[[length(results) + 1]] <- data.frame(
      participant_id = pid,
      model = "dual_kernel",
      metric = c("roc-auc", "pr-auc", "mcc"),
      value = c(as.numeric(roc_bvk), pr_bvk, mcc_bvk)
    )
    ...
    results[[length(results) + 1]] <- data.frame(
      ...
      model = "q_learning",
      metric = c("roc-auc", "pr-auc", "mcc"),
      value = c(as.numeric(roc_q), pr_q, mcc_q)
    )
  }
}
...
# Factor models for proper coloring
final_df$model <- factor(final_df$model, levels=c("q_learning", "dual_kernel"))

cat("Generating paired boxplot...\n")
pdf("figures/3_metrics.pdf", width = 11, height = 8.5)
p <- ggplot(final_df, aes(x = model, y = value)) +
  geom_boxplot(aes(fill = model), alpha = 0.5, outlier.shape = NA, width = 0.4) +
  geom_line(aes(group = participant_id), color = "gray", alpha = 0.5) +
  geom_jitter(aes(color = model), width = 0.05, height = 0, size = 2) +
  facet_wrap(~ metric, scales = "free_y") +
  scale_fill_viridis_d(direction = -1) +
  scale_color_viridis_d(direction = -1) +
  labs(title = "predictive switch metrics by participant", y = "metric value", x = "model") +
