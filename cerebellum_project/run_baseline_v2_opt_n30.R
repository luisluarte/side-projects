...
mod_v2_opt <- cmdstan_model("/home/DCCS5/cerebellum_project/src/stan/baseline_v2_opt.stan", cpp_options = list(stan_threads = TRUE))

cat("Sampling Baseline V2 (N=30) ...\n")
fit_v2_opt <- mod_v2_opt$sample(
  data = stan_data_v2_opt,
  chains = 4,
  parallel_chains = 2,
  threads_per_chain = 4,
  init = 0,
...
fit_v2_opt$save_output_files(dir = "/home/DCCS5/cerebellum_project/results/", basename = "fit_v2_opt_n30")

summ_v2 <- fit_v2_opt$summary()
cat("Baseline V2 Diagnostics -> Max Rhat:", max(summ_v2$rhat, na.rm=TRUE), "| Min Bulk ESS:", min(summ_v2$ess_bulk, na.rm=TRUE), "\n")
loo_v2_opt <- fit_v2_opt$loo()
saveRDS(loo_v2_opt, "/home/DCCS5/cerebellum_project/results/loo_v2_opt_n30.rds")

loo_m012 <- readRDS("/home/DCCS5/cerebellum_project/results/loo_m012_n30.rds")
cat("\n==========================================\n")
cat(" LOO-CV COMPARISON (N=30)\n")
cat("==========================================\n")
comp <- loo_compare(list(M012 = loo_m012, BaselineV2Opt = loo_v2_opt))
print(comp)
cat("\n")
...
tnd_cap <- pmin(stan_data_v2_opt$min_rt - 0.05, 3.69)
