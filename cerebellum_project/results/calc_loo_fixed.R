...
cat("Computing Base Log-Likelihood...\n")
for(d in 1:num_draws) {
  for(s in 1:N_subj) {
    ...
    for(t in 1:N_t) {
      ...
      # The drift rate (positive means towards boundary 1)
      veff_raw <- v_ctx * (Q[1]-Q[2])
      
      resp_str <- if(ch==1) "upper" else "lower"
      if (rt <= tnd) rt_eff <- tnd + 0.001 else rt_eff <- rt
      
      den <- dwiener(rt_eff, alpha=a, tau=tnd, beta=0.5, delta=veff_raw, resp=resp_str)
...
cat("Computing M009 Log-Likelihood...\n")
...
for(d in 1:num_draws) {
  for(s in 1:N_subj) {
    ...
    for(t in 1:N_t) {
      ...
      # Correct drift rate mapping: 
      # Positive veff_raw points toward Boundary 1 (Upper).
      ...
      den <- dwiener(rt_eff, alpha=a_dyn, tau=tnd, beta=0.5, delta=veff_raw, resp=resp_str)
      if(is.na(den) || is.nan(den) || den < 1e-10) den <- 1e-10
