...
cat("1. Updating repository...\n")
system("cd /home/DCCS5/cerebellum_project && git reset --hard && git pull")

cat("2. Compiling fixed Stan models...\n")
mod_base <- cmdstan_model("/home/DCCS5/cerebellum_project/src/stan/baseline.stan", cpp_options = list(stan_threads = TRUE))
mod_m006 <- cmdstan_model("/home/DCCS5/cerebellum_project/src/stan/m006_strict_hmc.stan", cpp_options = list(stan_threads = TRUE))
...
stan_data <- list(
  ...
  rt = test_dat$RT,
  iti = test_dat$ITI,
  min_rt = min_rt_df$min_rt - 0.05,
  start_idx = subj_indices$start_idx,
  end_idx = subj_indices$end_idx,
  W_exp = W_exp,
  theta_mean = rep(0.5, 9),
  L_Sigma = diag(9)
)

cat("3. Fitting Baseline...\n")
fit_base <- mod_base$sample(data = stan_data, chains = 4, parallel_chains = 4, threads_per_chain = 1, iter_warmup = 300, iter_sampling = 300)
fit_base$save_object("/home/DCCS5/cerebellum_project/results/baseline_final.rds")

cat("4. Fitting M006...\n")
fit_m006 <- mod_m006$sample(data = stan_data, chains = 4, parallel_chains = 4, threads_per_chain = 1, iter_warmup = 300, iter_sampling = 300, max_treedepth=10, adapt_delta=0.8)
fit_m006$save_object("/home/DCCS5/cerebellum_project/results/m006_final.rds")

cat("5. Forward Simulation Metrics...\n")
...
cat(sprintf("Effect Size (Cohen's d): %.3f\n", summary(m_roc)$coefficients["is_m006", "Estimate"] / sd_pooled_roc))
...
cat(sprintf("Effect Size (Cohen's d): %.3f\n", summary(m_pr)$coefficients["is_m006", "Estimate"] / sd_pooled_pr))
...
cat(sprintf("Effect Size (Cohen's d): %.3f\n", summary(m_mcc)$coefficients["is_m006", "Estimate"] / sd_pooled_mcc))
...
cat(sprintf("Effect Size (Cohen's d): %.3f\n", summary(m_rmse)$coefficients["is_m006", "Estimate"] / sd_pooled_rmse))

cat("6. Extracting LOO CV...\n")
log_lik_base <- fit_base$draws("log_lik", format = "matrix")
log_lik_m006 <- fit_m006$draws("log_lik", format = "matrix")
loo_base <- loo(log_lik_base)
loo_m006 <- loo(log_lik_m006)
cat("\nLOO Compare:\n")
print(loo_compare(loo_base, loo_m006))