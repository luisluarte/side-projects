...
for(s in 1:N_subj) {
    ...
    res_list[[length(res_list)+1]] <- data.frame(subj=s, model="Baseline", roc=b_roc, pr=b_pr, rmse=b_rmse)
    res_list[[length(res_list)+1]] <- data.frame(subj=s, model="M006", roc=m_roc, pr=m_pr, rmse=m_rmse)
}

df_res <- bind_rows(res_list) %>% filter(!is.na(roc))

cat("\nSummary_Table_N10_Fixed\n")
print(df_res %>% group_by(model) %>% summarise(mean_roc=mean(roc), mean_pr=mean(pr), mean_rmse=mean(rmse)))
