...
# Using the previously evaluated predictions that we saved in test_dat... wait, I didn't save test_dat to RDS.
# I will just run the evaluation function quickly again!
...
DataFrame eval_metrics(int N_trials, int N_subj, IntegerVector subj, IntegerVector resp, NumericVector reward, NumericVector rt, NumericVector iti,
    ...
    NumericVector b_pred_p1(N_trials), m_pred_p1(N_trials);
    std::vector<double> frac_alpha(32), kappa_vec(32);
    for(int i=0; i<32; ++i) { frac_alpha[i] = 0.1 + 0.8*(i/31.0); kappa_vec[i] = 0.1 + 0.89*(i/31.0); }
    ...
    for(int t=0; t<N_trials; ++t) {
        int s = subj[t] - 1; int ch = resp[t] - 1; double R = reward[t];
        
        double b_veff = b_v[s] * (b_Q[s*2 + 0] - b_Q[s*2 + 1]);
        b_pred_p1[t] = 1.0 / (1.0 + std::exp(-1.0 * b_veff * b_a[s]));
        ...
        for(int i=0; i<32; ++i) {
            double W_PC_eff = 3.0 * std::tanh(W_PC_latent[s][i] / 3.0);
            double eff_z = W_PC_eff * Z[s][i];
            double S_mask = std::tanh(m_gs[s] * std::sqrt(eff_z*eff_z + 1e-8));
            if(i < 16) cb0 += S_mask * eff_z; else cb1 += S_mask * eff_z;
        }
        
        double veff_scaled = (m_v[s] * 0.054) * m_Q_diff[s] + (m_g[s] * 0.054) * (cb0 - cb1);
        double m_veff = 18.51 * std::tanh(veff_scaled);
        double a_raw = m_a_raw[s] + m_wu[s] * std::sqrt((cb0*cb0+1e-8) * (cb1*cb1+1e-8));
        double m_a_dyn = 0.11 + 7.36 * (1.0 / (1.0 + std::exp(-a_raw)));
        
        m_pred_p1[t] = 1.0 / (1.0 + std::exp(-1.0 * m_veff * m_a_dyn));
    ...
    return DataFrame::create(_["b_pred_p1"] = b_pred_p1, _["m_pred_p1"] = m_pred_p1);
}")
...
preds <- eval_metrics(nrow(test_dat), N_subj, test_dat$subj_idx, test_dat$Boundary, test_dat$F, test_dat$RT, test_dat$ITI,
                      ...
                      m006_a_raw, m006_tnd, m006_v, m006_alpha, m006_alpha_pc, m006_gamma, m006_golgi, m006_tau, m006_wu, W_exp)
...
for(s in 1:N_subj) {
    ...
    # ROC for RAW CHOICE
    b_roc_choice <- calc_auc(s_dat$b_pred_p1, s_dat$Boundary == 1)
    m_roc_choice <- calc_auc(s_dat$m_pred_p1, s_dat$Boundary == 1)
    ...
    b_roc_switch <- calc_auc(s_dat$b_p_switch, s_dat$is_switch)
    m_roc_switch <- calc_auc(s_dat$m_p_switch, s_dat$is_switch)
    
    res_list[[length(res_list)+1]] <- data.frame(subj=s, model="Baseline", roc_choice=b_roc_choice, roc_switch=b_roc_switch)
    res_list[[length(res_list)+1]] <- data.frame(subj=s, model="M006", roc_choice=m_roc_choice, roc_switch=m_roc_switch)
}

df_res <- bind_rows(res_list) %>% filter(!is.na(roc_choice))
cat("\nMean ROC-AUC across subjects:\n")
df_res %>% group_by(model) %>% summarise(ROC_Choice = mean(roc_choice), ROC_Switch = mean(roc_switch)) %>% print()