...
DataFrame eval_metrics(int N_trials, int N_subj, IntegerVector subj, IntegerVector resp, NumericVector reward, NumericVector rt, NumericVector iti,
    ...
    for(int t=0; t<N_trials; ++t) {
        ...
        b_pred_rt[t] = b_tnd[s] + (b_a[s] / (2.0 * (b_veff != 0 ? std::abs(b_veff) : 1e-5))) * std::tanh((std::abs(b_veff) * b_a[s]) / 2.0);
        ...
        m_pred_rt[t] = m_tnd[s] + (m_a_dyn / (2.0 * (m_veff != 0 ? std::abs(m_veff) : 1e-5))) * std::tanh((std::abs(m_veff) * m_a_dyn) / 2.0);
    ...
    return DataFrame::create(Named(\"b_pred_p1\") = b_pred_p1, Named(\"b_pred_rt\") = b_pred_rt, Named(\"m_pred_p1\") = m_pred_p1, Named(\"m_pred_rt\") = m_pred_rt);
}
")
...
for(s in 1:N_subj) {
    ...
    # RAW CHOICE PREDICTION (Boundary == 1)
    # The models predict Probability of Boundary 1
    truth <- s_dat$Boundary == 1
    
    b_roc <- calc_auc(s_dat$b_pred_p1, truth)
    m_roc <- calc_auc(s_dat$m_pred_p1, truth)
    b_pr <- calc_pr_auc(s_dat$b_pred_p1, truth)
    m_pr <- calc_pr_auc(s_dat$m_pred_p1, truth)
    b_mcc <- calc_mcc(s_dat$b_pred_p1, truth)
    m_mcc <- calc_mcc(s_dat$m_pred_p1, truth)
