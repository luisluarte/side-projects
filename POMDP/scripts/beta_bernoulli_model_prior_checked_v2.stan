...
functions {
  real partial_log_lik(
      ...
      array[] int animal_slice,
                       ...
                       matrix r_animal_beta, matrix r_animal_kappa, matrix r_animal_phi, matrix r_animal_beta_slope, matrix r_animal_side,
    ...
    for (i in 1:(end - start + 1)) {
      ...
      // invididual side bias
      real trait_side = r_animal_side[animal_idx, 1];
      ...
      for (s in sessions_per_animal_start[animal_idx]:sessions_per_animal_end[animal_idx]) {
        ...
        real side_s = mu_side[d_idx, cog_ctx] + trait_side;
        ...
        for (t in start_idx[s]:end_idx[s]) {
          ...
          // this change side to be a zero sum scaling tip
          // that is if animal prefers right side and S amount then
          // it follow that is does NOT prefer the left side by an S amount
          // this should further separate the parameter from kappa
          // and also makes it so bias retains the combined expected value
          // of both spouts the same, so it affects the choice between the spouts
          // but does not influence the over all value of the taks, moving 
          // away the animal from WAIT state, so now its just a relative pref
          Q_base[ID_LICK1] = (ev1 * 1.0) + (side_s / 2.0);
          Q_base[ID_LICK2] = (ev2 * 1.0) - (side_s / 2.0);
...
parameters {
  ...
  // SBC determined that there was some issue with kappa and side
  // to make them more separate and constraining side bias to 1 global estimate
  // plus individual variance, but removing context to context variability
  // this makes theoretical sense as bias should be a strain-level thing
  real veh_shift_side;
  ...
  // adding the animal level trait for side
  real<lower=0> sigma_side_trait;
  ...
  // added side here also
  vector[N_animals] side_trait_raw;
}
...
transformed parameters {
  ...
  matrix[N_animals, 1] r_animal_side = to_matrix(sigma_side_trait * side_trait_raw, N_animals, 1);
  ...
  for (c in 1:N_cognitive_contexts) {
    ...
    // remove the per context estimation
    mu_side[2, c]         = base_side       + veh_shift_side;
    ...
    // remove the per context estimation
    mu_side[3, c]         = mu_side[2, c]       + drug_delta_side;
...
model {
 ...
 // but logically 0.5/0.25 > 100/50 if kappa matters at all
  ...
  target += reduce_sum(partial_log_lik, animal_indices, grainsize,
                       ...
                       mu_beta_slope,
                       r_animal_beta, r_animal_kappa, r_animal_phi, r_animal_beta_slope, r_animal_side,
                       epsilon,
                       N_actions, N_states,
